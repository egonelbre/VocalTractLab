...
#include "analysis/F0EstimatorYin.h"
#include <limits>
#include <cmath>
...
F0EstimatorYin::F0EstimatorYin()
{
  ...
  int windowLength = (int)(0.03*(double)AUDIO_SAMPLING_RATE_HZ);
  ...
  filter->createChebyshev(UPPER_CUTOFF_FREQ / (double)AUDIO_SAMPLING_RATE_HZ, false, 2);
  IirFilter highPass;
  highPass.createChebyshev(LOWER_CUTOFF_FREQ / (double)AUDIO_SAMPLING_RATE_HZ, true, 4);
...
void F0EstimatorYin::init(Signal16 *signal, int firstRoiSample, int numRoiSamples)
{
  ...
  int numFrames = (int)((double)signalLength / (double)(AUDIO_SAMPLING_RATE_HZ*INTERNAL_TIME_STEP_S));
...
bool F0EstimatorYin::processChunk(int numChunkSamples)
{
  ...
  int firstFrame = (int)(firstChunkSample / ((double)AUDIO_SAMPLING_RATE_HZ*INTERNAL_TIME_STEP_S));
  int lastFrame = (int)((lastChunkSample) / ((double)AUDIO_SAMPLING_RATE_HZ*INTERNAL_TIME_STEP_S));
...
//  printf("Processing frames %d to %d\n", firstFrame, lastFrame);
  ...
  {
    centerPos_pt = (int)(i*INTERNAL_TIME_STEP_S*(double)AUDIO_SAMPLING_RATE_HZ);
...
vector<double> F0EstimatorYin::finish()
{
  ...
  int numF0Values = (int)(origSignal.N / ((double)AUDIO_SAMPLING_RATE_HZ * timeStep_s));
...
void F0EstimatorYin::getFrameData(double *frameSignal, double *df, double *ndf, FrameData &fd)
{
  ...
  const int MIN_TAU = (int)((double)AUDIO_SAMPLING_RATE_HZ / MAX_F0);
  ...
  for (i=0; i < INTEGRATION_LENGTH; i++)
  {
        ...
        (ndf[minPos] <= 1.0))
    {
      ...
      fd.pitchCandidateT0[fd.numPitchCandidates] = accurateTau / (double)AUDIO_SAMPLING_RATE_HZ;
...
double F0EstimatorYin::getBestLocalT0Estimate(double t_s)
{
  ...
  int numSurroundingFrames = (int)((double)INTEGRATION_LENGTH / ((double)AUDIO_SAMPLING_RATE_HZ*INTERNAL_TIME_STEP_S));
