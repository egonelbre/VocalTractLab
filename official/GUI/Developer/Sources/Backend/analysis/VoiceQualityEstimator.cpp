...
#include "analysis/VoiceQualityEstimator.h"
...
void VoiceQualityEstimator::init(Signal16 *signal, int firstRoiSample, int numRoiSamples)
{
  ...
  slices.resize((unsigned int)(((double)signalLength / (double)AUDIO_SAMPLING_RATE_HZ) / SLICE_STEP_S));
  ...
  firstRoiSlice = (int)(((double)firstRoiSample / (double)AUDIO_SAMPLING_RATE_HZ) / SLICE_STEP_S);
  int lastRoiSlice = (int)(((double)(firstRoiSample + numRoiSamples) / (double)AUDIO_SAMPLING_RATE_HZ) / SLICE_STEP_S);
...
vector<double> VoiceQualityEstimator::finish()
{
  ...
  int numVqValues = (int)(origSignal.N / ((double)AUDIO_SAMPLING_RATE_HZ * timeStep_s));
...
{
  int sliceIndex = (int)(((double)pos_pt / (double)AUDIO_SAMPLING_RATE_HZ) / SLICE_STEP_S);
...
void VoiceQualityEstimator::calcSlicePeaks(int sliceIndex)
{
  ...
  int firstSample = (int)(startTime_s*AUDIO_SAMPLING_RATE_HZ);
  int lastSample  = (int)(endTime_s*AUDIO_SAMPLING_RATE_HZ) - 1;
  int i;
  double d;
  Slice *s = &slices[sliceIndex];
...
void VoiceQualityEstimator::calcWavelet(Signal &wavelet, int lengthFactor)
{
  ...
  int waveletLength_pt = (int)((double)AUDIO_SAMPLING_RATE_HZ * waveletLength_s);
  ...
  {
    t_s = (double)(-waveletLength_pt/2 + i) / (double)AUDIO_SAMPLING_RATE_HZ;
    t_s/= lengthFactor;
