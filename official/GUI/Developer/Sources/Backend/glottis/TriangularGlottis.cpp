...
TriangularGlottis::TriangularGlottis()
{
  ...
  Parameter sp[NUM_STATIC_PARAMS] =
  {
    ...
    { "L. rest thickness", "lower_rest_thickness", "cm", 10, "mm", 0.1, 0.5, 0.24, 0.0 },
    { "U. rest thickness", "upper_rest_thickness", "cm", 10, "mm", 0.01, 0.2, 0.06, 0.0 },
    { "L. rest mass", "lower_rest_mass", "g", 1, "g", 0.01, 0.2, 0.12, 0.0 },
    { "U. rest mass", "upper_rest_mass", "g", 1, "g", 0.01, 0.2, 0.03, 0.0 },
    ...
    { "L. contact spring k", "lower_contact_spring_k", "dyne/cm", 0.001, "kdyne/cm", 0.0, 500000.0, 240000.0, 0.0 },
    { "U. contact spring k", "upper_contact_spring_k", "dyne/cm", 0.001, "kdyne/cm", 0.0, 500000.0, 24000.0, 0.0 },
    { "Coupling spring k", "coupling_spring_k", "dyne/cm", 0.001, "kdyne/cm", 0.0, 400000.0, 25000.0, 0.0 },
    { "Inlet length", "inlet_length", "cm", 10, "mm", 0.0, 1.0, 0.05, 0.0 },
    { "Outlet length", "outlet_length", "cm", 10, "mm", 0.0, 0.5, 0.01, 0.0 },
    { "Natural F0", "natural_f0", "Hz", 1, "Hz", 20.0, 500.0, 129.0, 0.0 },
    { "dF0/dQ", "f0_div_q", "Hz", 1, "Hz", 20.0, 500.0, 125.51, 0.0 },
  };
  ...
  {
    { "Lower rel. displacement", "lower_rel_displacement", "cm", 10.0, "mm", -0.3, 0.3, 0.0, 0.0 },
    { "Upper rel. displacement", "upper_rel_displacement", "cm", 10.0, "mm", -0.3, 0.3, 0.0, 0.0 },
    { "Lower abs. displacement", "lower_abs_displacement", "cm", 10.0, "mm", 0.0, 1.0, 0.0, 0.0 },
    { "Upper abs. displacement", "upper_abs_displacement", "cm", 10.0, "mm", 0.0, 1.0, 0.0, 0.0 },
    { "Cord length", "cord_length", "cm", 10.0, "mm", 0.0, 3.0, 0.0, 0.0 },
    { "Lower thickness", "lower_thickness", "cm", 10.0, "mm", 0.0, 2.0, 0.0, 0.0 },
    { "Upper thickness", "upper_thickness", "cm", 10.0, "mm", 0.0, 2.0, 0.0, 0.0 },
    ...
    { "Contact area", "contact_area", "cm^2", 100, "mm^2", 0.0, 2.0, 0.0, 0.0 }
  };
...
//  double supraglottalPressure_dPa = supraglottalPressureFilter.getOutputSample(pressure_dPa[3]);
  ...
  restX[0] = controlParams[REST_DISP_1].x;
  restX[1] = controlParams[REST_DISP_2].x;
  ...
  mass[0] = staticParams[MASS_1].x / Q;
  mass[1] = staticParams[MASS_2].x / Q;
  ...
  springK[0] = staticParams[SPRING_K_1].x * Q;
  springK[1] = staticParams[SPRING_K_2].x * Q;
  contactSpringK[0] = staticParams[CONTACT_SPRING_K_1].x * Q;
  contactSpringK[1] = staticParams[CONTACT_SPRING_K_2].x * Q;
  ...
  double kc = staticParams[COUPLING_SPRING_K].x; // *Q*Q;
  ...
  dampingRatio[0] = staticParams[DAMPING_RATIO_1].x + alpha[0]*1.0;
  dampingRatio[1] = staticParams[DAMPING_RATIO_2].x + alpha[1]*1.0;
  ...
  double inletLength = staticParams[INLET_LENGTH].x;
  double outletLength = staticParams[OUTLET_LENGTH].x;
...
void TriangularGlottis::calcGeometry()
{
  ...
  relX[0] = relativeDisplacementBuffer[0][pos & BUFFER_MASK];
  relX[1] = relativeDisplacementBuffer[1][pos & BUFFER_MASK];
  backX[0] = controlParams[REST_DISP_1].x + relX[0];
  backX[1] = controlParams[REST_DISP_2].x + relX[1];

  // ****************************************************************
  // Area of the chink between the arytenoids.
  // ****************************************************************

  double chinkArea = controlParams[ARY_AREA].x;
  if (chinkArea < 0.0)
  {
    chinkArea = 0.0;
  }
  ...
  double Q = getTensionParameter( controlParams[FREQUENCY].x );
  double cordLength;
  double thickness[2];
  double area[2];
  double openLength[2];
  ...
  derivedParams[RELATIVE_DISP_1].x = relX[0];
  derivedParams[RELATIVE_DISP_2].x = relX[1];
  derivedParams[ABSOLUTE_DISP_1].x = backX[0];
  derivedParams[ABSOLUTE_DISP_2].x = backX[1];
  derivedParams[CURRENT_LENGTH].x = cordLength;
  derivedParams[CURRENT_THICKNESS_1].x = thickness[0];
  derivedParams[CURRENT_THICKNESS_2].x = thickness[1];
  derivedParams[CURRENT_AREA_1].x = area[0];
  derivedParams[CURRENT_AREA_2].x = area[1];
  derivedParams[CURRENT_TENSION].x = Q;
  derivedParams[CONTACT_AREA].x = contactArea;
}
...
{
  length_cm[0] = derivedParams[CURRENT_THICKNESS_1].x;
  length_cm[1] = derivedParams[CURRENT_THICKNESS_2].x;

  area_cm2[0] = derivedParams[CURRENT_AREA_1].x;
  area_cm2[1] = derivedParams[CURRENT_AREA_2].x;
}
...
{
  return controlParams[ASPIRATION_STRENGTH].x;
}
...
{
  double f0DivQ = staticParams[F0_DIV_Q].x;
  ...
  double Q = 1.0 + (f0 - staticParams[NATURAL_F0].x) / f0DivQ;
  if (Q < 0.05)
  {
    Q = 0.05;
  }
...
void TriangularGlottis::getLengthAndThickness(const double Q, double &length_cm, double thickness[])
{
  ...
  length_cm = staticParams[REST_LENGTH].x * factor;
  thickness[0] = staticParams[REST_THICKNESS_1].x / factor;
  thickness[1] = staticParams[REST_THICKNESS_2].x / factor;
}
