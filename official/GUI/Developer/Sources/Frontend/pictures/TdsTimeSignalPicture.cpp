...
void TdsTimeSignalPicture::draw(wxDC &dc)
{
  ...
  int posOffset = data->nextTdsSampleIndex;
  int y, lastY;
  double d;

  if (data->quantity == Data::QUANTITY_PRESSURE)
  {
    buffer = data->synthesizer->userProbePressure;
  }
  else
  if (data->quantity == Data::QUANTITY_FLOW)
  {
    buffer = data->synthesizer->userProbeFlow;
  }
  else
  if (data->quantity == Data::QUANTITY_AREA)
  {
    buffer = data->synthesizer->userProbeArea;
  }
  else
  {
  ...
  for (i=0; i < graphW; i++)
  {
    ...
    pos = (int)(t_s*(double)AUDIO_SAMPLING_RATE_HZ);   // relative sampling position
    pos+= posOffset;
    pos&= Synthesizer::TDS_BUFFER_MASK;                  // modulo buffer length
