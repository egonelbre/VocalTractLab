...
#include "pictures/VocalTractPicture.h"
#include "app/Data.h"
...
void VocalTractPicture::projection2D()
{
  ...
  glViewport(0, 0, (GLint)(width * scale), (GLint)(height * scale));
}
...
//    glClearColor(0.9f, 0.9f, 0.9f, 0);      // Light-gray background
  ...
  if ((showCenterLine) && (renderMode != RM_NONE))
  {
    ...
    // Die aktuelle Schnittfl�che einzeichnen ***********************
  ...
  if ((showControlPoints) && (renderMode != RM_NONE))
  {
    ...
    Point2D limitedTongueBody(tract->params[VocalTract::TCX].limitedX, tract->params[VocalTract::TCY].limitedX);
    Point2D limitedTongueTip(tract->params[VocalTract::TTX].limitedX, tract->params[VocalTract::TTY].limitedX);
  ...
  if (false)
  {
    ...
    Point3D P0, P1, n;
    Surface *s[7] = 
    { &tract->surfaces[VocalTract::UPPER_COVER], &tract->surfaces[VocalTract::LOWER_COVER],
      &tract->surfaces[VocalTract::UPPER_TEETH], &tract->surfaces[VocalTract::LOWER_TEETH],
      &tract->surfaces[VocalTract::UPPER_LIP],   &tract->surfaces[VocalTract::LOWER_LIP],
      &tract->surfaces[VocalTract::TONGUE] };
...
// ****************************************************************************
// Gibt zu den �bergebenen Bildschirmkoordinaten die Objektkoordinaten zur�ck, 
// die auf der �bergebenen Ebene liegen.
// ****************************************************************************
                      ...
                      const Point3D planePoint, const Point3D planeNormal)
{
  ...
  // Die Punkte P und Q r�ckw�rts transformieren
...
// ****************************************************************************
// Gibt den Punkt H auf dem Linienzug points[0 .. numPoints-1] zur�ck, der am  
...
Point3D VocalTractPicture::getMinDistancePoint(int screenX, int screenY, Point3D *point, int numPoints, int& bestIndex, double& bestT)
{
  ...
  {
    // Abstand vom St�tzpunkt zum Blickpunktstrahl
...
// screenY) und ver�ndert entsprechend die betroffenen Vokaltraktparameter.
...
//  int numPoints;
  ...
  switch(pointIndex)
  {
    ...
    case CP_JAW:
    {
      ...
      surface = &tract->surfaces[VocalTract::LOWER_COVER];
      ...
      if (radicand >= 0.0)
      {
        ...
        if ((S2 > -1.0) && (S2 < 1.0))
        {
          ...
          tract->params[VocalTract::JA].x = angle_rad*180.0 / M_PI;
          if ((angle_rad > -M_PI/4.0) && (angle_rad < M_PI/4.0))
          {
            tract->params[VocalTract::JX].x = (w.x + sin(angle_rad)*v.y) / cos(angle_rad) - v.x;
          }          
    ...
    case CP_HYOID:
    {
      ...
      surface = &tract->surfaces[VocalTract::NARROW_LARYNX_FRONT];
      narrowPoint = surface->getVertex(rib, ribPoint);

      surface = &tract->surfaces[VocalTract::WIDE_LARYNX_FRONT];
      widePoint = surface->getVertex(rib, ribPoint);

      double hyoidY = tract->surfaces[VocalTract::LOWER_COVER].getVertex(
      ...
      tract->params[VocalTract::HX].x = t;
      tract->params[VocalTract::HY].x = Q.y;
    ...
    case CP_LIP_DISTANCE:
    {
      ...
      Q = getObjectCoordinates(screenX, screenY, XY_PLANE_POINT, XY_PLANE_NORMAL);
      tract->params[VocalTract::LD].x = -2.0 * (Q.y - yClose + tract->params[VocalTract::JX].x);
    ...
    {
      Q = getObjectCoordinates(screenX, screenY, XY_PLANE_POINT, XY_PLANE_NORMAL);
      tract->params[VocalTract::TCX].x = Q.x;
      tract->params[VocalTract::TCY].x = Q.y;
    ...
    {
      Q = getObjectCoordinates(screenX, screenY, XY_PLANE_POINT, XY_PLANE_NORMAL);
      tract->params[VocalTract::TTX].x = Q.x;
      tract->params[VocalTract::TTY].x = Q.y;
    ...
    {
      Q = getObjectCoordinates(screenX, screenY, XY_PLANE_POINT, XY_PLANE_NORMAL);
      tract->params[VocalTract::TBX].x = Q.x;
      tract->params[VocalTract::TBY].x = Q.y;
    ...
    {
      Q = getObjectCoordinates(screenX, screenY, XY_PLANE_POINT, XY_PLANE_NORMAL);
      tract->params[VocalTract::TRX].x = Q.x;
      tract->params[VocalTract::TRY].x = Q.y;
      tract->restrictTongueParams();
    }
    break;
...
// ****************************************************************************
// Berechnet die Lage eines Kontrollpunktes in Objektkoordinaten in Abh�ngigkeit
...
void VocalTractPicture::parameterToControlPoint(int pointIndex)
{
  ...
  switch(pointIndex)
  {
    ...
    case CP_LIP_DISTANCE:
    {
      ...
      tract->getImportantLipPoints(onset, corner, F0, F1, yClose);
      x = F1.x;
      y = yClose - 0.5 * tract->params[VocalTract::LD].x - tract->params[VocalTract::JX].x;
      z = 0.0;
    }
    break;
    ...
    case CP_TONGUE_CENTER:
    {
      x = tract->params[VocalTract::TCX].x;
      y = tract->params[VocalTract::TCY].x;
      z = 0.0;
    }
    break;
    ...
    case CP_TONGUE_TIP:
    {
      x = tract->params[VocalTract::TTX].x;
      y = tract->params[VocalTract::TTY].x;
      z = 0.0;
    }
    break;
    ...
    case CP_TONGUE_BLADE:
    {
      x = tract->params[VocalTract::TBX].x;
      y = tract->params[VocalTract::TBY].x;
      z = 0.0;
    }
    break;
    ...
    case CP_TONGUE_BACK:
    {
      x = tract->params[VocalTract::TRX].x;
      y = tract->params[VocalTract::TRY].x;
      z = 0.0;
    }
    break;
...
// ****************************************************************************
// Schreibt die Daten zu einer Gitter-Oberfl�che in eine SVG-Datei.
// ****************************************************************************
    ...
    int firstRibPoint, int lastRibPoint, double minZ, double maxZ, bool localColors)
{
  ...
  {
    // Diejenige der verbleibenden Linien mit der gr��ten Tiefe raussuchen
    index = -1;
    maxDepth = 0.0;
...
bool VocalTractPicture::exportTractWireframeSVG(const wxString &fileName, int item)
{
  ...
  {
    // Larynx Hinterseite
    surface = &tract->surfaces[VocalTract::UPPER_COVER_TWOSIDE];
  ...
  {
    // Vorderseite Larynx
    surface = &tract->surfaces[VocalTract::LOWER_COVER_TWOSIDE];
    addSurfaceToSVG(os, surface, 
      0, VocalTract::NUM_LARYNX_RIBS-1, 0, surface->numRibPoints-1, minZ, maxZ, true);
  }
  else
  ...
  if (item == -1)
  {
    if (renderBothSides)
    {
      ...
      // Gesamte untere H�lle
      surface = &tract->surfaces[VocalTract::LOWER_COVER_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Obere Z�hne
      surface = &tract->surfaces[VocalTract::UPPER_TEETH_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Untere Z�hne
      surface = &tract->surfaces[VocalTract::LOWER_TEETH_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Lippen
      surface = &tract->surfaces[VocalTract::UPPER_LIP_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      surface = &tract->surfaces[VocalTract::LOWER_LIP_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Zunge
      surface = &tract->surfaces[VocalTract::TONGUE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Epiglottis
      surface = &tract->surfaces[VocalTract::EPIGLOTTIS_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Uvula     
      surface = &tract->surfaces[VocalTract::UVULA_TWOSIDE];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);
    }
    else
    {
      // Gesamte obere H�lle
      surface = &tract->surfaces[VocalTract::UPPER_COVER];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);
      ...
      // Obere Z�hne
      surface = &tract->surfaces[VocalTract::UPPER_TEETH];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Untere Z�hne
      surface = &tract->surfaces[VocalTract::LOWER_TEETH];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Lippen
      surface = &tract->surfaces[VocalTract::UPPER_LIP];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      surface = &tract->surfaces[VocalTract::LOWER_LIP];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);
      ...
      // Epiglottis
      surface = &tract->surfaces[VocalTract::EPIGLOTTIS];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);

      // Uvula     
      surface = &tract->surfaces[VocalTract::UVULA];
      addSurfaceToSVG(os, surface, 
        0, surface->numRibs-1, 0, surface->numRibPoints-1, minZ, maxZ, false);
    }
  }
...
bool VocalTractPicture::exportCrossSectionSVG(const wxString &fileName)
{
  ...
  // Horizontalen Strich dorthin malen, wo das obere das untere Profil ber�hrt
...
void VocalTractPicture::setProjectionMatrix3D(double fovy, double nearPlane, double farPlane)
{
  ...
  const double scale = this->GetContentScaleFactor();
  glViewport((GLint)(viewportX * scale), (GLint)(viewportY * scale),
             (GLint)(viewportWidth * scale), (GLint)(viewportHeight * scale));
  // Cache the viewport in logical pixels so gluProject() returns
  // coordinates that line up with wx mouse events.
  viewport[0] = (GLint)viewportX;
  viewport[1] = (GLint)viewportY;
  viewport[2] = (GLint)viewportWidth;
  viewport[3] = (GLint)viewportHeight;
}
...
void VocalTractPicture::setModelViewMatrix3D()
{
  ...
  // Dann die Matrix f�r die richtige Transformationsreihenfolge ****
...
void VocalTractPicture::setLights()
{
  ...
  // F�r korrekte Beleuchtung der R�ckseiten
...
void VocalTractPicture::renderSolid()
{
  ...
  Surface *tongue = &tract->surfaces[VocalTract::TONGUE];
  ...
  if (renderBothSides)
  {
	  transSurface[UPPER_TEETH] = &tract->surfaces[VocalTract::UPPER_TEETH_TWOSIDE];
	  transSurface[LOWER_TEETH] = &tract->surfaces[VocalTract::LOWER_TEETH_TWOSIDE];
    transSurface[UPPER_LIP]   = &tract->surfaces[VocalTract::UPPER_LIP_TWOSIDE];
    transSurface[LOWER_LIP]   = &tract->surfaces[VocalTract::LOWER_LIP_TWOSIDE];
    transSurface[UPPER_COVER] = &tract->surfaces[VocalTract::UPPER_COVER_TWOSIDE];
    transSurface[LOWER_COVER] = &tract->surfaces[VocalTract::LOWER_COVER_TWOSIDE];
    transSurface[LEFT_COVER]  = &tract->surfaces[VocalTract::LEFT_COVER];
    transSurface[RIGHT_COVER] = &tract->surfaces[VocalTract::RIGHT_COVER];
    transSurface[EPIGLOTTIS]  = &tract->surfaces[VocalTract::EPIGLOTTIS_TWOSIDE];
    transSurface[UVULA]       = &tract->surfaces[VocalTract::UVULA_TWOSIDE];
  }
  else
  {
    transSurface[UPPER_TEETH] = &tract->surfaces[VocalTract::UPPER_TEETH];
	  transSurface[LOWER_TEETH] = &tract->surfaces[VocalTract::LOWER_TEETH];
	  transSurface[UPPER_LIP]   = &tract->surfaces[VocalTract::UPPER_LIP];
    transSurface[LOWER_LIP]   = &tract->surfaces[VocalTract::LOWER_LIP];
    transSurface[UPPER_COVER] = &tract->surfaces[VocalTract::UPPER_COVER];
    transSurface[LOWER_COVER] = &tract->surfaces[VocalTract::LOWER_COVER];
    transSurface[LEFT_COVER]  = &tract->surfaces[VocalTract::LEFT_COVER];
    transSurface[RIGHT_COVER] = NULL;
    transSurface[EPIGLOTTIS]  = &tract->surfaces[VocalTract::EPIGLOTTIS];
    transSurface[UVULA]       = &tract->surfaces[VocalTract::UVULA];
  }
...
void VocalTractPicture::renderWireFrame()
{
  ...
  if (renderBothSides)
  {
    surface[0] = &tract->surfaces[VocalTract::UPPER_TEETH_TWOSIDE];
    surface[1] = &tract->surfaces[VocalTract::LOWER_TEETH_TWOSIDE];
    surface[2] = &tract->surfaces[VocalTract::UPPER_COVER_TWOSIDE];
    surface[3] = &tract->surfaces[VocalTract::LOWER_COVER_TWOSIDE];
    surface[4] = &tract->surfaces[VocalTract::EPIGLOTTIS_TWOSIDE];
    surface[5] = &tract->surfaces[VocalTract::UVULA_TWOSIDE];
    surface[6] = &tract->surfaces[VocalTract::UPPER_LIP_TWOSIDE];
    surface[7] = &tract->surfaces[VocalTract::LOWER_LIP_TWOSIDE];
    surface[8] = &tract->surfaces[VocalTract::TONGUE];
  }
  else
  {
    surface[0] = &tract->surfaces[VocalTract::UPPER_TEETH];
    surface[1] = &tract->surfaces[VocalTract::LOWER_TEETH];
    surface[2] = &tract->surfaces[VocalTract::UPPER_COVER];
    surface[3] = &tract->surfaces[VocalTract::LOWER_COVER];
    surface[4] = &tract->surfaces[VocalTract::EPIGLOTTIS];
    surface[5] = &tract->surfaces[VocalTract::UVULA];
    surface[6] = &tract->surfaces[VocalTract::UPPER_LIP];
    surface[7] = &tract->surfaces[VocalTract::LOWER_LIP];
    surface[8] = &tract->surfaces[VocalTract::TONGUE];
  }
...
//      glColor3f(0.5f, 0.5f, 1.0f);
  ...
  if (false)
  {
    s = &tract->surfaces[VocalTract::RADIATION];
...
wxString VocalTractPicture::getToolTipText(int controlPointIndex)
{
  ...
  if (controlPointIndex == CP_VELUM)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::VS].abbr.c_str(), tract->params[VocalTract::VS].x, 
      tract->params[VocalTract::VO].abbr.c_str(), tract->params[VocalTract::VO].x);
  }
  else
  if (controlPointIndex == CP_HYOID)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::HX].abbr.c_str(), tract->params[VocalTract::HX].x,
      tract->params[VocalTract::HY].abbr.c_str(), tract->params[VocalTract::HY].x);
  }
  else
  if (controlPointIndex == CP_JAW)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::JA].abbr.c_str(), tract->params[VocalTract::JA].x,
      tract->params[VocalTract::JX].abbr.c_str(), tract->params[VocalTract::JX].x);
  }
  else
  if (controlPointIndex == CP_LIP_CORNER)
  {
    st = wxString::Format("%s=%2.2f", 
      tract->params[VocalTract::LP].abbr.c_str(), tract->params[VocalTract::LP].x);
  }
  else
  if (controlPointIndex == CP_LIP_DISTANCE)
  {
    st = wxString::Format("%s=%2.2f", 
      tract->params[VocalTract::LD].abbr.c_str(), tract->params[VocalTract::LD].x);
  }
  else
  if (controlPointIndex == CP_TONGUE_CENTER)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::TCX].abbr.c_str(), tract->params[VocalTract::TCX].x,
      tract->params[VocalTract::TCY].abbr.c_str(), tract->params[VocalTract::TCY].x);
  }
  else
  if (controlPointIndex == CP_TONGUE_TIP)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::TTX].abbr.c_str(), tract->params[VocalTract::TTX].x,
      tract->params[VocalTract::TTY].abbr.c_str(), tract->params[VocalTract::TTY].x);
  }
  else
  if (controlPointIndex == CP_TONGUE_BLADE)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::TBX].abbr.c_str(), tract->params[VocalTract::TBX].x,
      tract->params[VocalTract::TBY].abbr.c_str(), tract->params[VocalTract::TBY].x);
  }
  else
  if (controlPointIndex == CP_TONGUE_BACK)
  {
    st = wxString::Format("%s=%2.2f\n%s=%2.2f", 
      tract->params[VocalTract::TRX].abbr.c_str(), tract->params[VocalTract::TRX].x,
      tract->params[VocalTract::TRY].abbr.c_str(), tract->params[VocalTract::TRY].x);
  }
...
void VocalTractPicture::OnMouseChanged(wxMouseEvent &event)
{
  ...
  {
    // Adjust the scaling of the background image.
  ...
  if (event.Dragging())
  {
    ...
    if (event.LeftIsDown())
    {
      ...
      if ((poster != NULL) && (showPoster) && (posterEditing))
      {
        ...
        // Reduce the dragging velocity of the background image.
        factor = factor * 0.5;
    ...
    // ****************************************************************
    // The right mouse button is pressed -> Move the model to the front
    // or back.
    // ****************************************************************
