...
void SimpleSpectrumPicture::draw(wxDC &dc)
{
  ...
  // of the average power spectrum (LTAS). The average spectrum is 
  // not calculated on the fly but only on request by the user.
  // ****************************************************************
...
void SimpleSpectrumPicture::drawFunction(wxDC &dc)
{
  ...
  if ((data->userSpectrumType == Data::CEPSTRUM) || (data->userSpectrumType == Data::TEST_SPECTRUM))
  {
    ...
    graph->abscissa.positiveLimit = (double)data->userSpectrum->N / (double)AUDIO_SAMPLING_RATE_HZ;
  ...
  if (graph == &timeSignalGraph)
  {
    ...
    int lastY = 0;
    double t_s;
    double timeStep_s = 1.0 / (double)AUDIO_SAMPLING_RATE_HZ;
    int index;
    double w;
    double value;
  ...
  // ****************************************************************
  {
    ...
    int lastY = 0;
    double f_Hz;
    double freqStep_Hz = (double)AUDIO_SAMPLING_RATE_HZ / (double)s->N;
    int index;
    double w;
    double value;
