...
{
  int x = VocalTract::NUM_PARAMS + (int)gs->glottis->controlParams.size();
  return x;
}
  ...
  for (i=0; i < N; i++)
  {
    ...
    if ((y+h >= 0) && (y < windowHeight))
    {
      ...
      for (k=0; k < numGestures; k++)
      {
        ...
        // Is this a return gesture from a consonant or from a velic gesture?
        isReturnGesture = false;
        if ((k > 0) && (gesture->neutral) &&
          (i >= GesturalScore::LIP_GESTURE) && (i <= GesturalScore::VELIC_GESTURE))
        {
          if (sequence->getGesture(k - 1)->neutral == false) 
          { 
            isReturnGesture = true;
          }
        }
        ...
        if ((leftX < windowWidth) && (rightX >= Data::LEFT_SCORE_MARGIN))
        {
          ...
          if ((i == data->selectedGestureType) && (k == data->selectedGestureIndex))
          {
            if (gesture->neutral)
            {
              dc.SetBrush( wxColor(255, 215, 0) );    // Light yellow
            }
            else
            {
              dc.SetBrush( wxColor(220, 185, 0) );    // Dark yellow
            }
          }
          else
          if (isReturnGesture)
          {
            dc.SetBrush(wxColor(248, 248, 248));    // Light gray
          }
          else
          if (gesture->neutral)
          {
          ...
          {
            dc.SetBrush( wxColor(230, 230, 230) );    // Dark gray
          }
          ...
          if (isReturnGesture)
          {
            ...
            if (gesture->neutral == false)
            {
              ...
              else
              {
                ...
                {
                  st = wxString::Format("%2.2f %s", gesture->dVal, sequence->unit.c_str());
                }
                else
                {
...
void GesturalScorePicture::paintMotorProgram(wxDC &dc)
{
  ...
  {
    minVal[i] = gs->vocalTract->params[i].min;
    maxVal[i] = gs->vocalTract->params[i].max;
  }
  ...
  for (i=VocalTract::NUM_PARAMS; i < N; i++)
  {
    ...
    // Should not happen:
    if (k >= (int)gs->glottis->controlParams.size())
    {
      k = (int)gs->glottis->controlParams.size() - 1;
    }
    minVal[i] = gs->glottis->controlParams[k].min;
    maxVal[i] = gs->glottis->controlParams[k].max;
  }
  ...
  for (i=0; i < N; i++)
  {
    ...
    if ((y+h >= 0) && (y < windowHeight))
    {
      if (i < VocalTract::NUM_PARAMS)
      {
        label = wxString( data->vocalTract->params[i].abbr );
      }
      else
      {
        label = gs->glottis->controlParams[i - VocalTract::NUM_PARAMS].name;
      }
...
{
  int numParams = VocalTract::NUM_PARAMS + (int)gs->glottis->controlParams.size();
  min = 0.0;
  max = 1.0;
  ...
  {
    min = gs->vocalTract->params[paramIndex].min;
    max = gs->vocalTract->params[paramIndex].max;
  }
  else
...

  {
    ...
    min = gs->glottis->controlParams[k].min;
    max = gs->glottis->controlParams[k].max;
    ...
    // For subglottal pressure, take the range limits of the gesture type.
    if (k == Glottis::PRESSURE)
    {
      min = gs->gestures[GesturalScore::PRESSURE_GESTURE].minValue;
      max = gs->gestures[GesturalScore::PRESSURE_GESTURE].maxValue;
    }
  }
...
// We should NOT require that STRG is pressed during dragging the
// mouse, because that saturates the event queue with keypress
// events and makes the dragging very slow...
//    if (event.ControlDown()) 
    {
      data->gesturalScoreMark_s = data->gsTimeAxisGraph->getAbsXValue(mx);
...
{
  if (data->segmentSequence->numSegments() == 0)
  {
    wxMessageBox("There are no segments in the sequence.", "Error");
    return;
  }

  Segment* s = data->segmentSequence->getSegment(0);
  if ((s->value[Segment::NAME_INDEX] != "") || (s->duration_s < 0.100))
  {
    wxMessageBox("The first segment must be a pause (empty string) of at least 100 ms length. Please correct it.", "Error");
    return;
  }
