...
void AnatomyParams::getFrom(VocalTract *tract)
{
  ...
  param[PHARYNX_LENGTH].x = a->palateHeight_cm[3] -
    0.5*(tract->params[VocalTract::HY].min + tract->params[VocalTract::HY].max);
...
//    widthScale = 1.0;
  ...
  VocalTract::Param *origParam = referenceVocalTract->params;
  VocalTract::Param *newParam  = tract->params;
...
void AnatomyParams::adjustTongueRootCalculation(VocalTract *targetVocalTract)
{
  ...
  for (i=0; i < VocalTract::NUM_PARAMS; i++)
  {
    origSavedParams[i] = referenceVocalTract->params[i].x;
    newSavedParams[i] = targetVocalTract->params[i].x;
  }
  ...
  for (i=0; i < VocalTract::NUM_PARAMS; i++)
  {
    referenceVocalTract->params[i].x = 0.5*(referenceVocalTract->params[i].min + referenceVocalTract->params[i].max);
    targetVocalTract->params[i].x = 0.5*(targetVocalTract->params[i].min + targetVocalTract->params[i].max);
  }
  referenceVocalTract->calculateAll();
  targetVocalTract->calculateAll();
  
  // Hyoid position.
  origH = referenceVocalTract->surfaces[VocalTract::LOWER_COVER].getVertex(VocalTract::NUM_LARYNX_RIBS-1, VocalTract::NUM_LOWER_COVER_POINTS-1).toPoint2D();
  newH = targetVocalTract->surfaces[VocalTract::LOWER_COVER].getVertex(VocalTract::NUM_LARYNX_RIBS-1, VocalTract::NUM_LOWER_COVER_POINTS-1).toPoint2D();
  ...
  for (i=0; i < VocalTract::NUM_PARAMS; i++)
  {
    referenceVocalTract->params[i].x = origSavedParams[i];
    targetVocalTract->params[i].x = newSavedParams[i];
  }
