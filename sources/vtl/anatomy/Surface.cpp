...
// Borders for the tile wall. W = H = 0.5 cm seems to be the optimal value.
...
void Surface::init(int ribs, int ribPoints)
{
  ...
  // Speicher f�r die Oberfl�chenpunkte und Normalen allokieren.
...
void Surface::calculateNormals()
{
  ...
  // Die Normalen aller Fl�chen berechnen und auf die Eckpunkte �bertragen.
  ...
  for (i=0; i < numTriangles; i++)
  {
    ...
    // Normalenvektor der Fl�che (L�nge ist proportional zum Fl�cheninhalt)

    triangle[i].planeNormal = crossProduct(v0, v1);
    length = triangle[i].planeNormal.magnitude();
    triangle[i].area = 0.5*length;

    // Den Fl�chennormalenvektor normieren.
    triangle[i].planeNormal.normalize();

    // Die Eckpunktnormalen werden mit der Fl�chennormalen initialisiert (skaliert mit Fl�cheninhalt).
    for (k=0; k < 3; k++)
    {
  ...
  for (i=0; i < numVertices; i++)
  {
    // Jedes Zweierp�rchen von Dreiecken an diesem Punkt �berpr�fen.
    for (j=0; j < vertex[i].numAssociates-1; j++)
    {
      ...
      for (k=j+1; k < vertex[i].numAssociates; k++)
      {
        ...
        {
          // Die Fl�chennormalen der Fl�chen j und k gegenseitig auf die Punkte addieren.
  ...
  // Alle Eckpunktnormalen auf die L�nge 1 bringen.
  ...
  for (i=0; i < numTriangles; i++)
  {
    ...
    for (x=leftTile; x <= rightTile; x++)
    {
      for (y=bottomTile; y <= topTile; y++)
      {
        ...
        {
          // Error !
          printf("ERROR in Surface::prepareIntersections(): Too many triangles for one tile!\n");
        }
      }
    }
  }
...
/// The function returns false, when MAX_ENTRIES is smaller than the actual
/// necessary number of entries.
/// Parameters:
/// indexList: The list to be filled with the triangle indices. Triangles
/// may occur multiple times in the list, when they overlap muliple tiles.
/// numEntries: Returns the number of list entries.
/// MAX_ENTRIES: The maximal number of entries to be put in the list.
// ****************************************************************************
...
void Surface::prepareIntersection(Point2D Q, Point2D v)
{
  ...
  // Abstand EPSILON links bzw. rechts der Gerade Q+t*v auf der H�he von
  // P liegen.
...
bool Surface::getTriangleIntersection(int index, Point2D &P0, Point2D &P1, Point2D &n)
{
  ...
  // Den Normalenvektor der Fl�che berechnen.
  ...
  // ****************************************************************
  // Die Projektion des Normalenvektors auf die Dreicksfl�che
  // Dazu muss lineVector die L�nge 1 haben !
  // ****************************************************************

  n.x = normal.z;
  n.y = normal.x*lineVector.x + normal.y*lineVector.y;

  // Bei zwei Schnittpunkten P0 und P1 zur�ckgeben.
