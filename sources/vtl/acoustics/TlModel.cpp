...
const double TlModel::MIN_AREA_CM2  = 0.001; // = 0.1 mm^2; should not be smaller - danger of number overflow
...
TlModel::TlModel()
{
  ...
  options.viscousLossForFlow  = true;
...
void TlModel::getImpulseResponse(Signal *impulseResponse, int lengthExponent)
{
  ...
  getSpectrum(RADIATION_CHARACTERISTIC, &radiationSpectrum, signalLength, 0);
...
void TlModel::getSpectrum(SpectrumType type, ComplexSignal *spectrum, int spectrumLength, int section)
{
  ...
  double newF0 = (double)AUDIO_SAMPLING_RATE_HZ / (double)spectrumLength;
  ...
  if ((options.innerLengthCorrections  != prevOptions.innerLengthCorrections) ||
      ...
      (options.viscousLossForFlow   != prevOptions.viscousLossForFlow) ||
  ...
  for (i=0; i < numFreq; i++)
  {
    ...
    if (type == RADIATION_CHARACTERISTIC) { v = radiationCharacteristic[i]; }
...
int TlModel::getMostConstrictedSection()
{
  ...
  {
    if (tube.sections[i]->area_cm2 <= tube.sections[k]->area_cm2) 
    { 
      k = i; 
    }
  }
...
{
  double A_g = tube.sections[ Tube::LOWER_GLOTTIS_SECTION ]->area_cm2;
  double A_c = tube.sections[getMostConstrictedSection()]->area_cm2;
  ...
  const int MAX_FORMANTS, bool &frictionNoise, bool &isClosure, bool &isNasal)
{
  ...
  // The spectral length of 2^11 = 2048 is a trade-off between speed
  // of calculation and frequency resolution (=22 Hz). Due to the low
  // frequency resolution, the bandwidths measurements are not very
  // accurate!
  ...
  int i, k;
  double f0 = (double)AUDIO_SAMPLING_RATE_HZ / (double)SPECTRUM_LENGTH;
  ...
  double oldGlottalArea0 = tube.sections[Tube::LOWER_GLOTTIS_SECTION]->area_cm2;
  double oldGlottalArea1 = tube.sections[Tube::LOWER_GLOTTIS_SECTION + 1]->area_cm2;

  tube.sections[Tube::LOWER_GLOTTIS_SECTION]->area_cm2 = 0.3;   // = 0.3 cm^2 (Glottal opening for fricatives)
  tube.sections[Tube::LOWER_GLOTTIS_SECTION + 1]->area_cm2 = 0.3;
  ...
  tube.sections[Tube::LOWER_GLOTTIS_SECTION]->area_cm2 = oldGlottalArea0;
  tube.sections[Tube::UPPER_GLOTTIS_SECTION]->area_cm2 = oldGlottalArea1;
  ...
  if (tube.sections[Tube::FIRST_NOSE_SECTION]->area_cm2 > NASALITY_THRESHOLD_CM2) 
  { 
    isNasal = true; 
  } 
  else 
...
void TlModel::prepareCalculations()
{
  ...
  for (i=0; i < numFreq; i++)
  {
    ...
    mouthRadiationImpedance[i]  = getRadiationImpedance(omega, tube.sections[Tube::LAST_MOUTH_SECTION]->area_cm2);
    noseRadiationImpedance[i] = getRadiationImpedance(omega, tube.sections[Tube::LAST_NOSE_SECTION]->area_cm2);
  ...
  double A_g = tube.sections[Tube::LOWER_GLOTTIS_SECTION]->area_cm2;    // glottal area
  int minAreaSection = getMostConstrictedSection();
  double A_c = tube.sections[minAreaSection]->area_cm2;
  ...
  if (tube.sections[Tube::FIRST_NOSE_SECTION]->area_cm2 > MIN_AREA_CM2) 
  { 
    meanFlow = 0.0; 
  }
  ...
  for (i=0; i < numFreq; i++)
  {
    ...
    for (k=Tube::FIRST_TRACHEA_SECTION; k <= Tube::LAST_PHARYNX_SECTION; k++)   
    {
      ...
      {
        M.unitMatrix();
        M.B = getJunctionImpedance(omega, tube.sections[k-1]->area_cm2, tube.sections[k]->area_cm2);
        K*= M;
      }
...
void TlModel::getLumpedSectionImpedances(double omega, Tube::Section *ts, ComplexValue &Za, ComplexValue &Zb)
{
  ...
  double dcResistance = DC_R0 * pow(REF_AREA_CM2 / area, DC_EXPONENT);
  double acResistance = AC_R0 * pow(REF_AREA_CM2 / area, AC_EXPONENT) * sqrt(AC_FREQUENCY_HZ);
  double Rs = acResistance;
  if ((dcResistance > acResistance) && (options.viscousLossForFlow))
  {
    Rs = dcResistance;
  }
  // Mulitiply the per-unit-length resistance with half the tube length.
  Rs *= 0.5 * length;   

  Za = ComplexValue(Rs, omega * Ls);
...
{
  Tube::Section *ts = tube.sections[section];
  ...
  if ((section >= Tube::FIRST_SINUS_SECTION) && (section <= Tube::LAST_SINUS_SECTION))
  {
    ...
    // This empirical factor should be the same as in TdsModel.cpp.
    // It generates a stronger damping of the pole-zero pairs from
    // the paranasal sinuses.
    const double EMPIRICAL_FACTOR = 15.0;
    double Rs = EMPIRICAL_FACTOR * (8.0 * AIR_VISCOSITY_CGS * M_PI * length) / (area * area);
  ...
  // ****************************************************************
  {
    ...
    if ((dcResistance > acResistance) && (options.viscousLossForFlow))
    {
      Rs = dcResistance;    // Per-unit-length values
    }

    // The entire impedance in series (per length-unit)
    ComplexValue z(Rs, omega * Ls);
...
ComplexValue TlModel::getFlowSourceTF(int freqIndex, int section)
{
  ...
  if ((section >= Tube::FIRST_TRACHEA_SECTION) && (section <= Tube::LAST_PHARYNX_SECTION))
  {
    ...
    getLumpedSectionImpedances(discreteOmega[freqIndex], tube.sections[section], Za, Zb);
    inputImpedance+= Za;
    outputImpedance+= Za;
    sourceImpedance = Zb;
