...
const double GesturalScore::DEFAULT_TIME_CONSTANT_S = 0.010;  // For 100 ms transition durations.

// The dissertation by Christian Kleiner and other studies 
// (e.g., Moll, 1970) indicate that velic closing is faster than opening.
const double GesturalScore::VELIC_OPENING_TIME_CONSTANT_S = 0.012;
const double GesturalScore::VELIC_CLOSING_TIME_CONSTANT_S = 0.010;
...
{
  return ((index >= 0) && (index < (int)gestures.size()));
}
...
int GestureSequence::getIndexAt(double pos_s)
{
  ...
  for (i=0; (i < (int)gestures.size()) && (k == -1); i++)
  {
    if ((pos_s >= start_s) && (pos_s < start_s + gestures[i].duration_s)) 
    { 
      k = i; 
    }
    start_s+= gestures[i].duration_s;
  }
...
double GestureSequence::getDuration_s()
{
  ...
  for (i=0; i < (int)gestures.size(); i++)
  {
    duration_s+= gestures[i].duration_s;
  }
...
void GestureSequence::insertGesture(Gesture &g, int index)
{
  ...
  vector<Gesture>::iterator iter = gestures.begin();
  advance(iter, index);
  gestures.insert(iter, g);
}
...
/// overwritten or skipped accordingly. The return value is the index of the
/// new gesture in the sequence.
// ****************************************************************************

int GestureSequence::putGesture(Gesture &g, double startPos_s)
{
  const double MIN_GESTURE_DURATION_S = (double)MIN_GESTURE_DURATION_MS / 1000.0;

  // Make the range checks.
  limitGestureParams(g);

  // End position of the new gesture.
  double endPos_s = startPos_s + g.duration_s;

  int i;
  int indexOfNewGesture = 0;
  ...
  for (i = 0; i < (int)gestures.size(); i++)
  {
    if ((startPos_s >= pos_s) && (startPos_s <= pos_s + gestures[i].duration_s))
    {
      leftGestureIndex = i;
      leftGestureStart_s = pos_s;
    }

    if ((endPos_s >= pos_s) && (endPos_s <= pos_s + gestures[i].duration_s))
    {
      rightGestureIndex = i;
      rightGestureStart_s = pos_s;
    }

    pos_s += gestures[i].duration_s;
  }
  ...
  if ((leftGestureIndex == -1) && (rightGestureIndex == -1))
  {
    ...
    {
      g.duration_s += emptyDuration_s;
    }

    // Now append the intended new gesture.
    appendGesture(g);
    indexOfNewGesture = (int)gestures.size() - 1;
  }
  else
  ...
  if (leftGestureIndex != -1)     // Should here always be the case.
  {
    ...
    {
      newGestures.push_back(gestures[i]);
    }

    // Append the beginning of the original gesture leftGestureIndex

    Gesture gesturePart = gestures[leftGestureIndex];
    ...
    indexOfNewGesture = (int)newGestures.size() - 1;

    if (rightGestureIndex != -1)
    {
      // Append the remaining part of the rightGestureIndex
      gesturePart = gestures[rightGestureIndex];
      gesturePart.duration_s = rightGestureStart_s + gestures[rightGestureIndex].duration_s - endPos_s;
      if (gesturePart.duration_s < MIN_GESTURE_DURATION_S)
      {
        // Extend the inserted gesture g by the tiny bit of length.
        newGestures.back().duration_s += gesturePart.duration_s;
      }
      else
      {
        newGestures.push_back(gesturePart);
      ...
      for (i = rightGestureIndex + 1; i < (int)gestures.size(); i++)
      {
        newGestures.push_back(gestures[i]);
      }
    }
  ...
  if (indexOfNewGesture < 0)
  {
    indexOfNewGesture = 0;
  }
  if (indexOfNewGesture >= (int)gestures.size())
  {
    indexOfNewGesture = (int)gestures.size() - 1;
  }

  return indexOfNewGesture;
}
...
bool GestureSequence::writeToXml(ostream &os, int indent)
{
  ...
  for (i=0; i < (int)gestures.size(); i++)
  {
    if (nominalValues)
    {
      value = gestures[i].sVal;
    }
    else
    {
      sprintf(st, "%f", gestures[i].dVal);
      value = string(st);
    }
    ...
    sprintf(st, "<gesture value=\"%s\" slope=\"%f\" duration_s=\"%f\" "
      ...
      value.c_str(),
      gestures[i].slope,
      gestures[i].duration_s,
      gestures[i].tau_s,
      (int)gestures[i].neutral);
...
GesturalScore::GesturalScore(VocalTract *vocalTract, Glottis *glottis)
{
  ...
  gestures[VOWEL_GESTURE].init("Vowel gestures", "vowel-gestures", "", 
    0.0, 0.0, 0.0, 0.0, 0.008, 0.020, true);
  ...
  gestures[VELIC_GESTURE].init("Velic gestures", "velic-gestures", "cm^2", 
    -0.1, 1.5, 0.0, 0.0, 0.005, 0.04, false);

  gestures[GLOTTAL_SHAPE_GESTURE].init("Glottal shape g.", "glottal-shape-gestures", "", 
    0.0, 0.0, 0.0, 0.0, 0.01, 0.04, true);

  // For f0 gestures, we need a very big range for the slope from
  // -200 to +200 st/s, because of the rapid f0 drops during glottal stops.
  // Normally, the maximum change of pitch is from -80 ... +80 st/s.
  gestures[F0_GESTURE].init("F0 gestures", "f0-gestures", "st",
    48.0, 108.0, -200.0, 200.0, 0.005, 0.04, false);
...
/// and their durations and f0 values).
// ****************************************************************************
...
void GesturalScore::createFromSegmentSequence(SegmentSequence *origSegmentSequence)
{
  ...
  // The f0 gestures are only re-created in the following function
  // when f0 values are given for at least one phoneme.
  createPitchGestures(origSegmentSequence);
  ...
  // ****************************************************************
  // If the there are multiple pause segments in sequence, make them
  // a single pause with the total length of the individual pauses.
  // Also make sure that there is a pause of at least 100 ms at the
  // beginning and end of the sequence.
  // ****************************************************************

  i = 0;
  Segment* currSegment = nullptr;
  Segment* nextSegment = nullptr;

  while (i < segmentSequence.numSegments() - 1)
  {
    currSegment = segmentSequence.getSegment(i);
    nextSegment = segmentSequence.getSegment(i + 1);
    if ((currSegment->value[Segment::NAME_INDEX] == "") &&
      (nextSegment->value[Segment::NAME_INDEX] == ""))
    {
      // The current segment gets the total duration.
      currSegment->duration_s = currSegment->duration_s + nextSegment->duration_s;
      // Delete the next segment.
      segmentSequence.deleteSegment(i + 1);
    }
    else
    {
      i++;
  ...
  // There must be a pause of at least 100 ms at the beginning.
  
  Segment* s = nullptr;
  static const double MIN_START_END_PAUSE_DURATION_S = 0.1;

  s = segmentSequence.getSegment(0);
  if (s->value[Segment::NAME_INDEX] != "")
  {
    Segment pause;
    pause.duration_s = MIN_START_END_PAUSE_DURATION_S;
    pause.value[Segment::NAME_INDEX] = "";
    segmentSequence.insertSegment(pause, 0);
  }
  else if (s->duration_s < MIN_START_END_PAUSE_DURATION_S)
  {
    s->duration_s = MIN_START_END_PAUSE_DURATION_S;
    printf("ATTENTION: The pause at the beginning of the phone sequence "
      "has been increased! This causes a time shift of the entire sequence.\n");
  }

  // If there is no pause segment at the end, add one.
  // If there is one, set its duration to 100 ms.

  s = segmentSequence.getSegment(segmentSequence.numSegments() - 1);
  if (s->value[Segment::NAME_INDEX] != "")
  {
    Segment pause;
    pause.duration_s = MIN_START_END_PAUSE_DURATION_S;
    pause.value[Segment::NAME_INDEX] = "";
    segmentSequence.appendSegment(pause);
  }
  else
  {
    s->duration_s = MIN_START_END_PAUSE_DURATION_S;
  ...
  // ****************************************************************
  // Create the lung pressure gestures.
  // ****************************************************************

  const double LUNG_PRESSURE_DPA = 8000.0;      // = 800 Pa
  const double PRESSURE_ONSET_TIME_CONSTANT_S = 0.010;    // for 100 ms onset
  const double PRESSURE_OFFSET_TIME_CONSTANT_S = 0.020;   // for 200 ms offset

  Gesture g;

  // Create a zero pressure gesture from beginning to end.
  g.duration_s = segmentSequence.getDuration_s();
  g.dVal = 0.0;
  g.neutral = false;
  g.slope = 0.0;
  g.sVal = "";
  g.tau_s = PRESSURE_OFFSET_TIME_CONSTANT_S;

  gestures[PRESSURE_GESTURE].appendGesture(g);

  // ****************************************************************
  // For all clusters of non-pause segments, create one pressure
  // gesture with 800 Pa. 
  // ****************************************************************
  ...
  while (i < numSegments)
  {
    // Find the next non-pause segment.
    while ((i < numSegments) && (segmentSequence.getSegment(i)->value[Segment::NAME_INDEX] == ""))
    { 
      i++; 
    }
    int firstAudibleSegment = i;

    // Find the next pause segment.
    while ((i < numSegments) && (segmentSequence.getSegment(i)->value[Segment::NAME_INDEX] != ""))
    {
      i++;
    }
    int lastAudibleSegment = i - 1;

    if ((firstAudibleSegment < numSegments) && (lastAudibleSegment < numSegments))
    {
      // Insert a pressure pulse from firstAudibleSegment to lastAudibleSegment.
      string firstAudibleSegmentName = segmentSequence.getSegment(firstAudibleSegment)->value[Segment::NAME_INDEX];
      string lastAudibleSegmentName = segmentSequence.getSegment(lastAudibleSegment)->value[Segment::NAME_INDEX];

      double lungGestureBegin_s = segmentSequence.getSegmentBegin_s(firstAudibleSegment);
      double lungGestureEnd_s = segmentSequence.getSegmentEnd_s(lastAudibleSegment);

      // ****************************************************************
      // Adjust the start time of the lung gesture, depending of the 
      // type of the first segment.
      // ****************************************************************

      if (firstAudibleSegmentName == "h")
      {
        lungGestureBegin_s -= 4.0 * PRESSURE_ONSET_TIME_CONSTANT_S;
      }
      else if ((Sampa::isPlosive(firstAudibleSegmentName)) && (!Sampa::isVoiced(firstAudibleSegmentName)))
      {
        // Voiceless plosive: P_sub starts 100 ms left from the middle of the plosive.
        lungGestureBegin_s = 0.5 * (segmentSequence.getSegmentBegin_s(firstAudibleSegment) +
          segmentSequence.getSegmentEnd_s(firstAudibleSegment)) -
          10.0 * PRESSURE_ONSET_TIME_CONSTANT_S;
      }
      else if (Sampa::isFricative(firstAudibleSegmentName))
      {
        lungGestureBegin_s -= 6.0 * PRESSURE_ONSET_TIME_CONSTANT_S;
      }
      else
      {
        lungGestureBegin_s -= 4.0 * PRESSURE_ONSET_TIME_CONSTANT_S;
      ...
      // ****************************************************************
      // Adjust the end time of the lung gesture, depending of the 
      // type of the first segment.
      // The gesture should end at a time so that the subsequent gesture 
      // that drives the pressure back to zero reaches a certain pressure 
      // value at the end of the last segment (e.g. 200 Pa for vowels).
      // ****************************************************************

      // The target pressue at the end of the final segment.
      // It will be set below based on the type of segment.
      double targetPressure_dPa = 0.0;

      if (Sampa::isFricative(lastAudibleSegmentName))
      {
        // Drop P_sub to 100 Pa at the end of the last segment.
        targetPressure_dPa = 1000.0;
      }
      else if (Sampa::isPlosive(lastAudibleSegmentName))
      {
        // Drop P_sub to 150 Pa at the end of the last segment.
        targetPressure_dPa = 1500.0;
      }
      else
        // Voiced sounds: vowels, lateral, nasals, ...
      {
        // Drop P_sub to 200 Pa at the end of the last segment.
        targetPressure_dPa = 2000.0;
      }

      double deltaTime_s = getTimeToStepResponseValue_s(
        (LUNG_PRESSURE_DPA - targetPressure_dPa) / LUNG_PRESSURE_DPA,
        PRESSURE_OFFSET_TIME_CONSTANT_S);
      lungGestureEnd_s-= deltaTime_s;

      // ************************************************************

      double duration_s = lungGestureEnd_s - lungGestureBegin_s;
      if (duration_s > MIN_GESTURE_DURATION_S)
      {
        // The 800 Pa gesture
        g.duration_s = duration_s;
        g.dVal = LUNG_PRESSURE_DPA;
        g.neutral = false;
        g.slope = 0.0;
        g.sVal = "";
        g.tau_s = PRESSURE_ONSET_TIME_CONSTANT_S;

        gestures[PRESSURE_GESTURE].putGesture(g, lungGestureBegin_s);
      }
  ...
  // ****************************************************************
  // Create neutral consonantal gestures up to the end of the last
  // valid segment. This ensures that there is always a return 
  // gesture when consonantal gestures are inserted later on.
  // ****************************************************************
  ...
  gestures[LIP_GESTURE].appendGesture(g);
  gestures[TONGUE_TIP_GESTURE].appendGesture(g);
  gestures[TONGUE_BODY_GESTURE].appendGesture(g);
  gestures[VELIC_GESTURE].appendGesture(g);

  // ****************************************************************
  // Create a schwa vowel gesture up to the end of the sequence.
  // ****************************************************************
  ...
  string segmentName;
  int prevVowelIndex = -1;  // The previous vowel in the sequence (-1 = invalid)
  // Where did the previous V1-V2 movement end?
  double prevMovementEnd_s = 0.0;
  double segmentBegin_s = 0.0;
  double segmentEnd_s = 0.0;
  double segmentDur_s = 0.0;
  ...
  for (i = 0; i < numSegments; i++)
  {
    ...
    // If the segment is a pause, reset the index of the previous vowel,
    // so that vowels are not "connected" across pauses.
    if (segmentName == "")
    {
      prevVowelIndex = -1;
      continue;
    ...
    // to our study on secondary diphtongues:
    ...
    // **************************************************************
    // Generate a vowel gesture for a single vowel.
    // **************************************************************

    if (isVowel)
    {
      // Determine the following values depending on the condition.
      // t1_s: Start of movement from previous to this vowel.
      // t2_s: Target of this vowel reached (end of transition).

      double t1_s = 0.0;
      double t2_s = 10.0 * DEFAULT_TIME_CONSTANT_S;

      // This is the first vowel since the start or since the last pause.
      
      if (prevVowelIndex == -1)
      {
        t1_s = segmentBegin_s - 10.0 * DEFAULT_TIME_CONSTANT_S;
        t2_s = segmentBegin_s;
      }

      // The previous vowel was immediately before this vowel.

      else if (prevVowelIndex == i - 1)
      {
        // Split the transition equally between this and the previous vowel.
        t1_s = segmentBegin_s - 5.0 * DEFAULT_TIME_CONSTANT_S;
        t2_s = segmentBegin_s + 5.0 * DEFAULT_TIME_CONSTANT_S;
      }

      else

      // There is a transition from a previous vowel to this one
      // with one or more consonants inbetween.
      {
        // Calculate the relative timing of the vowel-vowel transition
        // according to Kleiner et al. (2015): "Timing of tongue body ..."

        double prevVowelBegin_s = segmentSequence.getSegmentBegin_s(prevVowelIndex);
        double prevVowelEnd_s = segmentSequence.getSegmentEnd_s(prevVowelIndex);
        double prevVowelDur_s = prevVowelEnd_s - prevVowelBegin_s;
        double consonantDur_s = segmentBegin_s - prevVowelEnd_s;  // Maybe 0.0 if there is no consonant.

        t1_s = prevVowelEnd_s - 0.035 + 0.208 * consonantDur_s - 0.441 * prevVowelDur_s;
        t2_s = segmentBegin_s + 0.076 - 0.462 * consonantDur_s;

        // t1 may not be more than 50 ms left of the previous vowel's end
        // or be further left than the middle of the previous vowel.

        if (t1_s < prevVowelEnd_s - 0.050)
        {
          t1_s = prevVowelEnd_s - 0.050;
        }

        if (t1_s < prevVowelBegin_s + 0.5 * prevVowelDur_s)
        {
          t1_s = prevVowelBegin_s + 0.5 * prevVowelDur_s;
        }

        // The transition should not reach more than 10 ms into the new vowel.

        if (t2_s > segmentBegin_s + 0.010)
        {
          t2_s = segmentBegin_s + 0.010;
        }

        // Max. transition duration is 200 ms.
      ...
      // Prevent that we cut short a previous V1-V2 transition.

      if (t1_s < prevMovementEnd_s)
      {
        t1_s = prevMovementEnd_s;
      }
      ...
      // Insert the new vowel gesture reaching from t1 to the end of
      // the vowel segment.
      // The time constant is 1/10 of the transition duration.

      g.duration_s = segmentEnd_s - t1_s;
      g.dVal = 0.0;
      g.neutral = false;
      g.slope = 0.0;
      g.sVal = vowelName;
      g.tau_s = 0.1 * (t2_s - t1_s);

      gestures[VOWEL_GESTURE].putGesture(g, t1_s);

      prevMovementEnd_s = t2_s;
    }

    // **************************************************************
    // Generate a two vowel gestures for a diphthong.
    // **************************************************************

    else if (isDiphthong)
    {
      // ************************************************************
      // These calculations follow the analysis of diphthong 
      // transitions in our paper:
      // Fietkau et al. (2022): "Relationship between the acoustic 
      // time intervals and tongue movements of German diphthongs"
      // ************************************************************

      double segmentDuration_s = segmentEnd_s - segmentBegin_s;
      double segmentMiddle_s = segmentBegin_s + 0.5 * segmentDuration_s;

      // The duration of the articulatory diphthong transition is linearly
      // related to the acoustic segment duration as follows:
      double diphthongTransitionDuration_s = 0.080 + 0.317 * segmentDuration_s;

      double t1_s = 0.0;    // Start of movement from previous vowel to V1.
      double t2_s = 0.0;    // End of movement to V1 and beginning of movement to V2.
      double t3_s = 0.0;    // Target of V2 reached.

      t2_s = segmentMiddle_s - 0.5 * diphthongTransitionDuration_s;
      t3_s = segmentMiddle_s + 0.5 * diphthongTransitionDuration_s;

      // This is the first vowel since the start or since the last pause.

      if (prevVowelIndex == -1)
      {
        t1_s = t2_s - 10.0 * DEFAULT_TIME_CONSTANT_S;
        double alternativeT1_s = segmentBegin_s + 0.010 - 10.0 * DEFAULT_TIME_CONSTANT_S;
        if (alternativeT1_s < t1_s)
        {
          t1_s = alternativeT1_s;
        }
      }
      ...
      {
        t1_s = t2_s - 10.0 * DEFAULT_TIME_CONSTANT_S;
        double prevVowelMiddle_s = 0.5 *
          (segmentSequence.getSegmentBegin_s(prevVowelIndex) + segmentSequence.getSegmentEnd_s(prevVowelIndex));
        if (t1_s < prevVowelMiddle_s)
        {
          t1_s = prevVowelMiddle_s;
        }
      }

      // There is a transition from a previous vowel to V1 of the 
      // diphthong with one or more consonants inbetween.
      else
      {
        // 20 ms before the end of the previous vowel.
        t1_s = segmentSequence.getSegmentEnd_s(prevVowelIndex) - 0.020;
        // Max. transition duration of 200 ms.
        if (t2_s - t1_s > 0.200)
        {
          t1_s = t2_s - 0.200;
        }
      ...
      setLowerLimit(t1_s, 0.0);
      setLowerLimit(t2_s, 0.0);
      setLowerLimit(t3_s, 0.0);
      ...
      // Gesture from the previous vowel to the first diphthong vowel.
      // The max. transition duration is 100 ms.

      double tau_s = 0.1 * (t2_s - t1_s);
      setUpperLimit(tau_s, DEFAULT_TIME_CONSTANT_S);

      g.duration_s = t2_s - t1_s;
      g.dVal = 0.0;
      g.neutral = false;
      g.slope = 0.0;
      ...
      // Gesture from first to second diphthong vowel.

      g.duration_s = segmentEnd_s - t2_s;
      g.dVal = 0.0;
      g.neutral = false;
      g.slope = 0.0;
      g.sVal = diphthongName[1];
      g.tau_s = 0.1 * (t3_s - t2_s);
      gestures[VOWEL_GESTURE].putGesture(g, t2_s);

      prevMovementEnd_s = t3_s;
    }

    // The current vowel index i becomes the previous vowel index.
    prevVowelIndex = i;
  }     // End of segment loop.
  ...
  // ****************************************************************
  // Run through all segments and create the FRICATIVES,
  // the LATERAL, the GLOTTAL STOP, and the GLOTTAL FRICATIVE.
  // ****************************************************************

  for (i = 1; i < numSegments - 1; i++)     // Skip the initial and final pause segments
  {
    s = segmentSequence.getSegment(i);
    segmentName = s->value[Segment::NAME_INDEX];
    segmentBegin_s = segmentSequence.getSegmentBegin_s(i);
    ...
    // **************************************************************
    // Extend the segment begin or end into potential pause segments
    // that neighbor the current segment, so that the corresponding
    // gestures are extended.
    // **************************************************************
    ...
    {
      segmentEnd_s = segmentSequence.getDuration_s();
    }
    else if ((i > 0) && (segmentSequence.getSegment(i - 1)->value[Segment::NAME_INDEX] == ""))
    {
      segmentBegin_s = 0.5 * (segmentSequence.getSegmentBegin_s(i - 1) + 
        segmentSequence.getSegmentEnd_s(i - 1));
    }
    else if ((i < numSegments - 1) && (segmentSequence.getSegment(i + 1)->value[Segment::NAME_INDEX] == ""))
    {
      segmentEnd_s = 0.5 * (segmentSequence.getSegmentBegin_s(i + 1) +
        segmentSequence.getSegmentEnd_s(i + 1));
    }
    ...
    // Audible segments are the "non-pause" segments.
    string nextAudibleSegmentName = "";

    if (segmentSequence.getSegment(i + 1) != NULL)
    {
      nextSegmentName = segmentSequence.getSegment(i + 1)->value[Segment::NAME_INDEX];
      nextAudibleSegmentName = nextSegmentName;
      // Skip a potential pause segment.
      if (nextAudibleSegmentName == "")
      {
        if (segmentSequence.getSegment(i + 2) != NULL)
        {
          nextAudibleSegmentName = segmentSequence.getSegment(i + 2)->value[Segment::NAME_INDEX];
        }
      }
    }
    ...
    if (isFricative)
    {
      double timeConstant_s = DEFAULT_TIME_CONSTANT_S;
      double transitionDuration_s = 10.0 * timeConstant_s;
      double halfTransitionDuration_s = 0.5 * transitionDuration_s;

      // Oral gesture begin must be a few ms earlier.
      double oralGestureBegin_s = segmentBegin_s - halfTransitionDuration_s - 0.010;
      double oralGestureEnd_s = segmentEnd_s - halfTransitionDuration_s;
      
      setLowerLimit(oralGestureBegin_s, 0.0);
      setLowerLimit(oralGestureEnd_s, 0.0);

      // If the next segment is a PLOSIVE with a different primary
      // articulator (e.g. in /naxt/ or /nask/), extend the oral
      // fricative gesture so that the oral constriction is still
      // there when the closure for the plosive is made. To do this,
      // we extend the fricative gesture by 40 ms (empirical value). 
      // Without this, there is often a noise burst at the implosion, 
      // because the vocal tract flow resitance is too low (open glottis).

      if ((Sampa::isPlosive(nextSegmentName)) &&
        (Sampa::getPrimaryArticulator(segmentName) != Sampa::getPrimaryArticulator(nextSegmentName)))
      {
        oralGestureEnd_s += 0.040;
      }

      // ************************************************************
      // Add the oral gestures: the constriction forming gesture and
      // the return gesture.
      // ************************************************************
      ...
      g.tau_s = timeConstant_s;

      gestures[gestureType].putGesture(g, oralGestureBegin_s);

      g.duration_s = transitionDuration_s;
      g.dVal = 0.0;
      g.neutral = true;   // The return gesture
      g.slope = 0.0;
      g.sVal = oralGestureName;
      g.tau_s = timeConstant_s;

      gestures[gestureType].putGesture(g, oralGestureEnd_s);
      ...
      double glottalGestureBegin_s = segmentBegin_s - halfTransitionDuration_s;
      double glottalGestureEnd_s = segmentEnd_s - halfTransitionDuration_s;

      // If the current segment is a VOICELESS fricative and the next
      // segment is a VOICELESS plosive, extend the glottal gesture 
      // by 40 ms (empirical value) so that there will be no 
      // potential trace of voicing between the two segments.

      if ((Sampa::isVoiced(segmentName) == false) &&
        ((nextAudibleSegmentName == "p") || 
          (nextAudibleSegmentName == "t") || 
          (nextAudibleSegmentName == "k")))
      {
        glottalGestureEnd_s += 0.040;
      }

      setLowerLimit(glottalGestureBegin_s, 0.0);
      setLowerLimit(glottalGestureEnd_s, 0.0);
    ...
    if (segmentName == "h")
    {
      ...
      setLowerLimit(gestureBegin_s, 0.0);
      setLowerLimit(gestureEnd_s, 0.0);
    ...
    if (segmentName == "l")
    {
      // /l/ needs short transition durations for sounding good.
      const double TRANSITION_DURATION_S = 0.080;
      double t1_s = segmentBegin_s - 0.5 * TRANSITION_DURATION_S;
      double t2_s = segmentEnd_s - 0.5 * TRANSITION_DURATION_S;
      double t3_s = segmentEnd_s + 0.5 * TRANSITION_DURATION_S;

      oralGestureName = "tt-alveolar-lateral";
      if (isPostalveolar[i])    // there is a /S,Z/ in the neighborhood
      {
        oralGestureName = "tt-postalveolar-lateral";
      }
      ...
      g.slope = 0.0;
      g.sVal = oralGestureName;
      g.tau_s = 0.1 * TRANSITION_DURATION_S;

      gestures[TONGUE_TIP_GESTURE].putGesture(g, t1_s);

      g.duration_s = t3_s - t2_s;
      g.dVal = 0.0;
      g.neutral = true;   // Return gesture.
      g.slope = 0.0;
      g.sVal = "";
      g.tau_s = 0.1 * TRANSITION_DURATION_S;

      gestures[TONGUE_TIP_GESTURE].putGesture(g, t2_s);
    }

  }   // End of segment loop

  // The fricatives etc. are now finished.


  // ****************************************************************
  // Run through all segments and create the PLOSIVES and NASALS.
  ...
  for (i = 1; i < numSegments - 1; i++)
  {
    ...
    segmentBegin_s = segmentSequence.getSegmentBegin_s(i);
    segmentEnd_s = segmentSequence.getSegmentEnd_s(i);
    segmentDur_s = segmentEnd_s - segmentBegin_s;

    // **************************************************************
    // Obtain the *modified* segment begin or end, which extends into 
    // potential pause segments that neighbor the current segment.
    // **************************************************************

    double modSegmentBegin_s = segmentBegin_s;
    double modSegmentEnd_s = segmentEnd_s;

    if (i == 1)
    {
      modSegmentBegin_s = 0.0;
    }
    else if (i == numSegments - 2)
    {
      modSegmentEnd_s = segmentSequence.getDuration_s();
    }
    else if ((i > 0) && (segmentSequence.getSegment(i - 1)->value[Segment::NAME_INDEX] == ""))
    {
      modSegmentBegin_s = 0.5 * (segmentSequence.getSegmentBegin_s(i - 1) +
        segmentSequence.getSegmentEnd_s(i - 1));
    }
    else if ((i < numSegments - 1) && (segmentSequence.getSegment(i + 1)->value[Segment::NAME_INDEX] == ""))
    {
      modSegmentEnd_s = 0.5 * (segmentSequence.getSegmentBegin_s(i + 1) +
        segmentSequence.getSegmentEnd_s(i + 1));
    }

    // **************************************************************
    // Get the name of the previous and next segments.
    // **************************************************************

    string prevSegmentName = "";
    string nextSegmentName = "";
    // Audible segments are the "non-pause" segments.
    string prevAudibleSegmentName = "";
    string nextAudibleSegmentName = "";

    if (segmentSequence.getSegment(i - 1) != NULL)
    {
      prevSegmentName = segmentSequence.getSegment(i - 1)->value[Segment::NAME_INDEX];
      prevAudibleSegmentName = prevSegmentName;
      // Skip a potential pause segment.
      if (prevAudibleSegmentName == "")
      {
        if (segmentSequence.getSegment(i - 2) != NULL)
        {
          prevAudibleSegmentName = segmentSequence.getSegment(i - 2)->value[Segment::NAME_INDEX];
        }
      }
    }
    ...
    GestureType gestureType = LIP_GESTURE;
    double oralGestureOnsetTau_s = DEFAULT_TIME_CONSTANT_S;
    double oralGestureOffsetTau_s = DEFAULT_TIME_CONSTANT_S;
    double targetVelicOpening_cm2 = 0.3;    // According to Steven's book, it is 0.2 to 0.3 cm^2.
    // How long does velic opening extend into vowels next to the nasal?
    double velicOpeningExtension_s = 0.0;   
    ...
    // The voice onset times were analyzed from the BITS-US corpus 
    // (where VOT and closure of plosives were individually segmented)
    // and fitted with 2nd order polynomials (see project Plosive-VOT-2025).
    // For the time constants for gesture formation and release, we
    // follow Kohler (1980), who found asymmetric velocities for
    ...
    if (segmentName == "p")
    {
      ...
      double x = segmentDur_s;
      if (x > 0.150) { x = 0.150; }
      voiceOnsetTime_s = 1.577 * x * x + 0.215 * x;
      oralGestureOnsetTau_s = 0.008;
      oralGestureOffsetTau_s = 0.014;
    }
...
//      glottalGestureName = "voiced-plosive";
      glottalGestureName = "modal";   // This is an exception here!

      // Exception here with VOT=0. With a VOT > 0, /b/ in /aba/ or /ibi/ may sound like /d/.
      voiceOnsetTime_s = 0.0;
      oralGestureOnsetTau_s = 0.010;
      oralGestureOffsetTau_s = 0.006;
    }
    ...
    if (segmentName == "t")
    {
      ...
      double x = segmentDur_s;
      if (x > 0.150) { x = 0.150; }
      voiceOnsetTime_s = 1.641 * x * x + 0.343 * x;
      oralGestureOnsetTau_s = 0.008;
      oralGestureOffsetTau_s = 0.014;
    }
    ...
    if (segmentName == "d")
    {
      ...
      double x = segmentDur_s;
      if (x > 0.100) { x = 0.100; }
      voiceOnsetTime_s = -2.430 * x * x + 0.483 * x;
      oralGestureOnsetTau_s = 0.010;
      oralGestureOffsetTau_s = 0.006;
    }
    ...
    if (segmentName == "k")
    {
      ...
      double x = segmentDur_s;
      if (x > 0.150) { x = 0.150; }
      voiceOnsetTime_s = 1.338 * x * x + 0.387 * x;
      oralGestureOnsetTau_s = 0.008;
      oralGestureOffsetTau_s = 0.014;
    }
    ...
    if (segmentName == "g")
    {
      ...
      double x = segmentDur_s;
      if (x > 0.100) { x = 0.100; }
      voiceOnsetTime_s = -1.867 * x * x + 0.507 * x;
      oralGestureOnsetTau_s = 0.010;
      oralGestureOffsetTau_s = 0.006;
    }
    ...
    if (segmentName == "m")
    {
      ...
      gestureType = LIP_GESTURE;
      oralGestureName = "ll-labial-closure";
      targetVelicOpening_cm2 = 0.3;
      oralGestureOnsetTau_s = 0.012;
      oralGestureOffsetTau_s = 0.012;
      
      velicOpeningExtension_s = 0.020;
    }
    ...
    if (segmentName == "n")
    {
      ...
      // /n/ sounds better when the velic opening is smaller than for /m,N/.
      targetVelicOpening_cm2 = 0.2;
      oralGestureOnsetTau_s = 0.012;
      oralGestureOffsetTau_s = 0.012;
      
      velicOpeningExtension_s = 0.030;
    }
    ...
    if (segmentName == "N")
    {
      ...
      velicOpeningExtension_s = 0.010;    // Shorter than for /m,n/.
    }
    ...
    if (isPlosive)
    {
      ...
      setLowerLimit(voiceOnsetTime_s, 0.0);

      // If the next segment is a fricative with the same primary 
      // articulator as the plosive (e.g. /pf/, /tS/), set VOT = 5 ms,
      // and make the plosive release very fast.
      string a = segmentName;
      string b = nextSegmentName;
      if (((a == "p" || a == "b") && (b == "f" || b == "v")) ||
        ((a == "t" || a == "d") && (b == "s" || b == "z" || b == "S" || b == "Z" || b == "C" || b == "j")) ||
        ((a == "k" || a == "g") && (b == "x" || b == "r" || b == "R" || b == "C" || b == "j")))
      {
        voiceOnsetTime_s = 0.005;
        oralGestureOffsetTau_s = 0.006;
      }

      double closureBegin_s = modSegmentBegin_s;  // May reach into a pause left from this segment.
      double closureEnd_s = segmentEnd_s - voiceOnsetTime_s;

      setLowerLimit(closureBegin_s, 0.0);

      // Make sure that the closure duration is at least 10 ms.
      const double MIN_CLOSURE_DURATION_S = 0.010;
      ...
      double glottalGestureBegin_s = closureBegin_s - 0.025;
      double glottalGestureEnd_s = modSegmentEnd_s - 0.065;

      // Glottal gesture should at least be 20 ms long, which is
      // needed that vocal fold oscillation can really stop.

      if (glottalGestureEnd_s - glottalGestureBegin_s < 0.020)
      {
        glottalGestureEnd_s = glottalGestureBegin_s + 0.020;
      }

      if (Sampa::isVoiced(segmentName))
      {
        // For voiced plosives: start adduction a little earlier.
        glottalGestureEnd_s = segmentEnd_s - 0.080;
      }

      // Shorten the glottal gesture if the next segment is a voiced fricative.
      if ((Sampa::isFricative(nextAudibleSegmentName)) && (Sampa::isVoiced(nextAudibleSegmentName)))
      {
      ...
      // Ensure a minimum glottal gesture duration for plosives so
      // that there is always a minimum of aspiration noise.

      const double MIN_GLOTTAL_GESTURE_DURATION_S = 0.010;
      if (glottalGestureEnd_s < glottalGestureBegin_s + MIN_GLOTTAL_GESTURE_DURATION_S)
      {
        glottalGestureEnd_s = glottalGestureBegin_s + MIN_GLOTTAL_GESTURE_DURATION_S;
      }

      // Add the oral closing gesture.

      bool connectToPrevGesture = (oralGestureName == prevOralGestureName);
      addClosingGesture(gestureType, oralGestureName,
        closureBegin_s, closureEnd_s,
        oralGestureOnsetTau_s, oralGestureOffsetTau_s,
        connectToPrevGesture);
      ...
      if (isNasal)
      {
        double closureBegin_s = modSegmentBegin_s;
        double closureEnd_s = modSegmentEnd_s;

        // Default for vowels.
        double velicOpeningBegin_s = modSegmentBegin_s - velicOpeningExtension_s;
        double velicOpeningEnd_s = modSegmentEnd_s + velicOpeningExtension_s;

        // Some special cases when plosives or fricatives are the
        // neighbouring segments.

        if (Sampa::isPlosive(prevAudibleSegmentName))
        {
          velicOpeningBegin_s = modSegmentBegin_s - 0.020;
        }
        if (Sampa::isPlosive(nextAudibleSegmentName))
        {
          velicOpeningEnd_s = modSegmentEnd_s;
        }

        if (Sampa::isFricative(prevAudibleSegmentName))
        {
          velicOpeningBegin_s = modSegmentBegin_s - 0.005;
        }
        if (Sampa::isFricative(nextAudibleSegmentName))
        {
          velicOpeningEnd_s = modSegmentEnd_s + 0.005;
        }

        if (prevAudibleSegmentName == "l")
        {
          velicOpeningBegin_s = modSegmentBegin_s - 0.005;
        }
        if (nextAudibleSegmentName == "l")
        {
          velicOpeningEnd_s = modSegmentEnd_s + 0.005;
        }

        setLowerLimit(closureBegin_s, 0.0);
        setLowerLimit(velicOpeningBegin_s, 0.0);
        ...
        addVelicOpeningGesture(velicOpeningBegin_s, velicOpeningEnd_s, targetVelicOpening_cm2);
      }
...
// ****************************************************************************
// Creates the sequence of pitch gestures from f0 values given for the phonemes
// in the given segment sequence, if there are any.
// The f0 values are specified in the attributes "f0_20%_Hz", "f0_50%_Hz", and
// "f0_80%_Hz" in the segments.
// ****************************************************************************

void GesturalScore::createPitchGestures(SegmentSequence* segmentSequence)
{
  struct Node
  {
    string name;    // Name of the phoneme.
    int percent;    // 20, 50 or 80%.
    double time_s;
    double f0_Hz;
  };

  static const double MIN_F0_HZ = 1.0;
  Node node;
  vector<Node> nodes;
  int numSegments = segmentSequence->numSegments();
  Segment* segment = nullptr;
  double startPos_s = 0.0;     // Start position of the segment in the sequence.
  double duration_s = 0.0;
  double f0_Hz = 0.0;
  string st;
  int i;
  ...
  // **************************************************************
  // Collect the time-value pairs (nodes) where f0 values are given.
  // Put in nodes with f0 = 0.0 when the value is not specified.
  // **************************************************************

  for (i = 0; i < numSegments; i++)
  {
    segment = segmentSequence->getSegment(i);
    duration_s = segment->duration_s;
    node.name = segment->value[Segment::NAME_INDEX];

    if (node.name == "?")
    {
      // The glottal stop is special. Insert three nodes at 0, 50, and 100%,
      // for which the f0 values are generated further below.
      node.f0_Hz = 0.0;
      
      node.percent = 0;
      node.time_s = startPos_s;
      nodes.push_back(node);

      node.percent = 50;
      node.time_s = startPos_s + 0.5 * duration_s;
      nodes.push_back(node);

      node.percent = 100;
      node.time_s = startPos_s + duration_s;
      nodes.push_back(node);
    }

    // Process all other segments if they are not empty (pauses).
    else if (node.name.empty() == false)
    {
      // **************************************************************

      try
      {
        f0_Hz = std::stod(segment->getValue("f0_20%_Hz"));
      }
      catch (...)
      {
        f0_Hz = 0.0;
      ...
      node.percent = 20;
      node.time_s = startPos_s + 0.2 * duration_s;
      node.f0_Hz = f0_Hz;
      nodes.push_back(node);
      ...
      try
      {
        f0_Hz = std::stod(segment->getValue("f0_50%_Hz"));
      }
      catch (...)
      {
        f0_Hz = 0.0;
      ...
      try
      {
        f0_Hz = std::stod(segment->getValue("f0_80%_Hz"));
      }
      catch (...)
      {
        f0_Hz = 0.0;
      ...
      node.percent = 80;
      node.time_s = startPos_s + 0.8 * duration_s;
      node.f0_Hz = f0_Hz;
      nodes.push_back(node);
    }

    startPos_s += duration_s;
  }

  // ****************************************************************
  // If there is no valid f0 node, return. Else clear the old pitch 
  // gestures.
  // ****************************************************************
  ...
  // Clear the pitch gestures.
  gestures[F0_GESTURE].clear();

  // ****************************************************************
  // Generate linearly interpolated f0 values at nodes where the f0 
  // is not defined.
  // ****************************************************************
  ...
  int leftIndex = 0;
  int rightIndex = 0;
  double leftF0_Hz = 0.0;
  double rightF0_Hz = 0.0;
  double leftTime_s = 0.0;
  double rightTime_s = 0.0;
  double timeDiff_s = 0.0;

  for (i = 0; i < N; i++)
  {
    if (nodes[i].f0_Hz < MIN_F0_HZ)
    {
      // Find the next valid nodes to the left and right.
      leftIndex = i;
      while ((leftIndex > 0) && (nodes[leftIndex].f0_Hz < MIN_F0_HZ)) { leftIndex--; }

      rightIndex = i;
      while ((rightIndex < N - 1) && (nodes[rightIndex].f0_Hz < MIN_F0_HZ)) { rightIndex++; }

      leftF0_Hz = nodes[leftIndex].f0_Hz;
      rightF0_Hz = nodes[rightIndex].f0_Hz;
      leftTime_s = nodes[leftIndex].time_s;
      rightTime_s = nodes[rightIndex].time_s;

      // Interpolate the f0 between the valid left and right neighbor nodes.
      if ((leftF0_Hz >= MIN_F0_HZ) && (rightF0_Hz >= MIN_F0_HZ))
      {
        timeDiff_s = rightTime_s - leftTime_s;
        if (timeDiff_s < 0.000001) { timeDiff_s = 0.000001; }
        nodes[i].f0_Hz = leftF0_Hz + (rightF0_Hz - leftF0_Hz) * (nodes[i].time_s - leftTime_s) / timeDiff_s;
      }

      // Only the left neighbor node is valid.
      else if (leftF0_Hz >= MIN_F0_HZ)
      {
        nodes[i].f0_Hz = leftF0_Hz;
      }

      // Only the right neighbor node is valid.
      else if (rightF0_Hz >= MIN_F0_HZ)
      {
        nodes[i].f0_Hz = rightF0_Hz;
      }
      else
      {
        // Should never happen !!!
  ...
  // ****************************************************************
  // Generate an f0 dip in the middle of glottal stops.
  // ****************************************************************
  ...
  for (i = 1; i < N - 1; i++)
  {
    if (nodes[i].name == "?")
    {
      if (nodes[i].percent == 0)
      {
        // The f0 should not be higher than its left neighbor node
        // (from the previous phoneme) as this might lead to a zig-zag f0.
        if (nodes[i].f0_Hz > nodes[i - 1].f0_Hz)
        {
          nodes[i].f0_Hz = nodes[i - 1].f0_Hz;
        }
      }
      else if (nodes[i].percent == 100)
      {
        // The f0 should not be higher than its right neighbor node
        // (from the next phoneme) as this might lead to a zig-zag f0.
        if (nodes[i].f0_Hz > nodes[i + 1].f0_Hz)
        {
          nodes[i].f0_Hz = nodes[i + 1].f0_Hz;
        }
      }
      else if (nodes[i].percent == 50)
      {
        double phoneDuration_s = nodes[i + 1].time_s - nodes[i - 1].time_s;
        double oldF0_st = getF0_st(nodes[i].f0_Hz);     // f0 in semitones

        // Reduce f0 by 1 st per every 10 ms duration.
        double newF0_st = oldF0_st - phoneDuration_s * 100.0;

        // The minimum is 50 st = 40 Hz.
        if (newF0_st < 50.0)
        {
          newF0_st = 50.0;
        }
        nodes[i].f0_Hz = getF0_Hz(newF0_st);
      }
    }
  }

  // ****************************************************************
  // Add one f0 gestures for each f0 node.
  // ****************************************************************

  static const double EPSILON = 0.000001;
  Gesture g;
  double f0_st = 0.0;
  double prevF0_st = 0.0;    // Prev. f0 value in semitones.

  for (i = 0; i < N; i++)
  {
    f0_st = getF0_st(nodes[i].f0_Hz);

    if (i == 0)
    {
      // Try to make the first gesture duration 10 ms shorter to 
      // compensate for the samll delay by the dynamical system with tau = 5 ms.
      g.duration_s = nodes[i].time_s - 0.010;
      if (g.duration_s < EPSILON)
      {
        g.duration_s = EPSILON;
      }
      g.dVal = f0_st;
      g.slope = 0.0;
    }
    ...
    {
      g.duration_s = nodes[i].time_s - nodes[i - 1].time_s;
      if (g.duration_s < EPSILON)
      {
        g.duration_s = EPSILON;
      }
      g.dVal = prevF0_st;
      g.slope = (f0_st - prevF0_st) / g.duration_s;
    }
    g.sVal = "";
    g.tau_s = 0.005;    // 5 ms (the smallest possible time constant)
    g.neutral = false;
...
// ****************************************************************************
/// Put a new closing gesture (and the return gesture) into the score and 
/// adjust its boundaries such that the actual closure starts and ends at the 
/// given times. When connectToPrevGesture = true, the beginning of the new 
/// gesture is set to the end of the last non-neutral gesture.
// ****************************************************************************
  ...
  bool connectToPrevGesture)
{
  ...
  const double MAX_GESTURE_DURATION_S = 2.0; // 0.4;

  // Range check: The closure duration must be at least 10 ms.
  if (closureEnd_s < closureBegin_s + 0.010)
  {
    closureEnd_s = closureBegin_s + 0.010;
  }
  ...
  while ((connectToPrevGesture == false) && (hasClosure == false) && (hasError == false))
  {
    ...
    hasClosure = hasVocalTactClosure(gestureType, gestureName,
      gestureBegin_s, gestureEnd_s, onsetTau_s, offsetTau_s, closureBegin_s);
  ...
  hasClosure = hasVocalTactClosure(gestureType, gestureName,
    gestureBegin_s, gestureEnd_s, onsetTau_s, offsetTau_s, closureEnd_s);
  ...
  if (hasClosure)
  {
    ...
    // or if the gesture has become too short, undo the last step left.
    ...
    // If the vocal tract is still closed at its intended offset, we
    // now incrementally reduce the offset time constant.

    const double DELTA_TAU_S = 0.0005;    // 0.5 ms
    while ((hasOffsetClosure) && (offsetTau_s >= 0.005 + DELTA_TAU_S))
    {
      offsetTau_s -= DELTA_TAU_S;
  ...
  // ****************************************************************
  // Put the closing and return gestures into the tier.
  // ****************************************************************

  putClosingGesture(gestureType, gestureName, gestureBegin_s, gestureEnd_s,
    onsetTau_s, offsetTau_s);
}


// ****************************************************************************
/// This function returns true if the following condition is fulfilled
/// at testTime_s, when the given oral gesture and its return gesture are put 
/// into the score: There is an oral closure with the primary articulator 
/// associated with the gestureType (lips, tongue tip, or tongue back).
// ****************************************************************************
...
// ****************************************************************************
// Put a consonantal closing gesture and a return gesture at the given position
// with the given time constants.
// ****************************************************************************

void GesturalScore::putClosingGesture(GestureType gestureType, string gestureName,
  double gestureBegin_s, double gestureEnd_s, double onsetTau_s, double offsetTau_s)
{
  static Gesture g;

  // ****************************************************************
  // Put the closing gesture into the tier.
  // ****************************************************************
  ...
  int index = gestures[gestureType].putGesture(g, gestureBegin_s);

  index++;    // Index of the following gesture

  // ****************************************************************
  // Put in the new return gesture. The default length of the return
  // gesture is 10.0 * offsetTau_s, but it may not be longer than the
  // sequence of neutral gestures that follows the consonantal gesture.
  // ****************************************************************

  // Get the length of the sequence of following neutral gestures.

  int N = gestures[gestureType].numGestures();
  bool doContinue = true;
  double durationOfNeutralGestures = 0.0;
  while ((index < N) && (doContinue))
  {
    Gesture* g = gestures[gestureType].getGesture(index);
    if (g->neutral)
    {
      durationOfNeutralGestures += g->duration_s;
    }
    else
    {
      doContinue = false;
  ...
  double returnGestureDuration_s = 10.0 * offsetTau_s;
  if (returnGestureDuration_s > durationOfNeutralGestures)
  {
    returnGestureDuration_s = durationOfNeutralGestures;
  }

  g.duration_s = returnGestureDuration_s;
  g.dVal = 0.0;
  g.neutral = true;
  g.slope = 0.0;
  g.sVal = "";
  g.tau_s = offsetTau_s;
  gestures[gestureType].putGesture(g, gestureEnd_s);
}
...
/// This function returns true if the following condition is fulfilled
/// at testTime_s, when the given oral gesture and its return gesture are put 
/// into the score: There is an oral constriction (15 mm^2) with the primary 
/// articulator associated with the gestureType (lips, tongue tip, or tongue back).
// ****************************************************************************

bool GesturalScore::hasVocalTactConstriction(GestureType gestureType, string gestureName,
  double gestureBegin_s, double gestureEnd_s,
  double onsetTau_s, double offsetTau_s,
  double testTime_s)
{
  ...
  // tongue back starts
  const double TONGUE_TIP_REGION_CM = 2.2;
  const double AREA_THRESHOLD_CM2 = 0.1501;   // 15 mm^2 + a very small increment.
  double pos_cm;
  ...
  for (i = 0; i < Tube::NUM_PHARYNX_MOUTH_SECTIONS; i++)
  {
    ...
    if (ts->area_cm2 <= AREA_THRESHOLD_CM2)
    {
      // Position in the middle of the closed tube section.
      pos_cm = ts->pos_cm + 0.5 * ts->length_cm;
...
{
  const double TIME_STEP_S = 1.0 / (double)CURVE_SAMPLING_RATE;
  const double MAX_GESTURE_DURATION_S = 5.0;
  ...
  double gestureBegin_s = openingBegin_s;
  double gestureEnd_s = openingEnd_s + 0.100;   // Very long time after the gesture begin.
  ...
  hasOpening = hasVelicOpening(gestureBegin_s, gestureEnd_s, targetVelicOpening_cm2, openingBegin_s);
  ...
  while ((hasOpening == false) && (hasError == false))
  {
    ...
    if ((gestureBegin_s < 0.010) || (gestureEnd_s - gestureBegin_s > MAX_GESTURE_DURATION_S))
    {
      if (gestureEnd_s - gestureBegin_s > MAX_GESTURE_DURATION_S)
      {
        printf("WARNING in GesturalScore::addVelicOpeningGesture().\n");
        // Take the initial time for the gesture begin.
  ...
  // Init the gesture end at 10 ms after the beginning.
  gestureEnd_s = gestureBegin_s + 0.010;
  ...
  hasOpening = hasVelicOpening(gestureBegin_s, gestureEnd_s, targetVelicOpening_cm2, openingEnd_s);
  hasError = false;
  ...
  // ****************************************************************
  // Put the new gesture and the return gesture into the tier.
  // ****************************************************************

  g.duration_s = gestureEnd_s - gestureBegin_s;
  g.dVal = targetVelicOpening_cm2;
  g.neutral = false;
  g.slope = 0.0;
  g.sVal = "";
  g.tau_s = VELIC_OPENING_TIME_CONSTANT_S;

  gestures[VELIC_GESTURE].putGesture(g, gestureBegin_s);

  g.duration_s = 10.0 * VELIC_CLOSING_TIME_CONSTANT_S;
  g.dVal = 0.0;
  g.neutral = true;     // The return gesture
  g.slope = 0.0;
  g.sVal = "";
  g.tau_s = VELIC_CLOSING_TIME_CONSTANT_S;

  gestures[VELIC_GESTURE].putGesture(g, gestureEnd_s);
}


// ****************************************************************************
/// Returns true, if the velic port is open at testTime_s, when a velic gesture
/// starting at gestureBegin_s and ending at gestureEnd_s, and its return 
/// gesture, are put into the tier for velic gestures.
// ****************************************************************************

bool GesturalScore::hasVelicOpening(double gestureBegin_s, double gestureEnd_s, 
  double targetVelicOpening_cm2, double testTime_s)
{
  const double VELIC_OPENING_THRESHOLD_CM2 = 0.01;  // Minimally open port.
  ...
  if (tractParams[VocalTract::VO] >= VELIC_OPENING_THRESHOLD_CM2)
  {
    hasOpening = true;
  }
...
bool GesturalScore::loadGesturesXml(const string &fileName, bool &allValuesInRange)
{
  ...
  // Recalculate all parameter curves.
  calcCurves();
...
void GesturalScore::calcCurves()
{
  ...
  for (i=0; i < VocalTract::NUM_PARAMS; i++)
  {
    neutral = vocalTract->params[i].neutral;
  ...
  {
    neutral = glottis->controlParams[i].neutral;
  ...
  double *consonantParams)
{
  ...
  if ((consonantIndexA == -1) || (consonantIndexI == -1) || (consonantIndexU == -1))
  {
    ...
    for (i=0; i < VocalTract::NUM_PARAMS; i++)
    {
      consonantParams[i] = vt->params[i].neutral;
    }
    return false;
  }
  ...
  const char *consonantName, const char *contextVowelName, double *consonantParams)
{
  ...
  if (shapeIndex == -1)
  {
    ...
    {
      contextVowelParams[k] = vt->params[k].neutral;
    }
  }
  else
  {
  ...
  double *vocalTractParams, GestureType gestureType)
{
  ...
  for (i = 0; i < VocalTract::NUM_PARAMS; i++)
  {
    vt->params[i].x = vocalTractParams[i];
  }
  vt->calculateAll();
...
// ****************************************************************************
/// Returns the complete tube geometry at the given position.
// ****************************************************************************

void GesturalScore::getTube(int sampleIndex, Tube& tube)
{
  int i;
  double pos_s = (double)sampleIndex / (double)AUDIO_SAMPLING_RATE_HZ;
  ...
  if (leftTubeIndex != neededLeftIndex)
  {
    if ((leftTubeIndex != -1) && (neededLeftIndex == leftTubeIndex + 1))
    {
      ...
      for (i = 0; i < VocalTract::NUM_PARAMS; i++)
      {
        vocalTract->params[i].x = tractParamCurve[i][neededLeftIndex + 1];
      }
      vocalTract->calculateAll();
    ...
    else
    {
      ...
      for (i = 0; i < VocalTract::NUM_PARAMS; i++)
      {
        vocalTract->params[i].x = tractParamCurve[i][neededLeftIndex];
      }
      vocalTract->calculateAll();
...
// ****************************************************************************
// Returns the control params of the glottis at the given sample index.
// ****************************************************************************

void GesturalScore::getGlottisParams(int sampleIndex, double* params)
{
  double pos_s = (double)sampleIndex / (double)AUDIO_SAMPLING_RATE_HZ;
  int numGlottisParams = (int)glottis->controlParams.size();

  getParams(pos_s, NULL, params);
}
...
{
  return (int)(getScoreDuration_s() * AUDIO_SAMPLING_RATE_HZ);
}
...
void GesturalScore::calcTractParamTargets()
{
    ...
    (index[4] < sequence[4]->numGestures()))
  {
    ...
    // **************************************************************
    // Add a gesture slice from pos_s to the end of the found 
    // gesture.
    // **************************************************************
      ...
      if (nearestPos_s > pos_s)
      {
        ...
        for (i=0; i < NUM_TRACT_GESTURE_TYPES; i++)
        {
          if (index[i] < sequence[i]->numGestures())
          {
            ...
            // If this a neutral consonantal or velic gesture, and
            // the previous gesture is non-neutral, name the slice
            // as "RET" to indicate a "return phase".
            if ((i > 0) && (g.neutral) && (index[i] > 0) &&
              (sequence[i]->getGesture(index[i] - 1)->neutral == false))
            {
              g.sVal = "RET";
            }
          }
  ...
  {
    // **************************************************************
    // Init. the target values with the vowel gesture shape.
    // **************************************************************
    ...
    if (shapeIndex == -1)
    {
      for (k=0; k < VocalTract::NUM_PARAMS; k++)
      {
        targetValue[k] = vocalTract->params[k].neutral;
    ...
    // Initialize with average target values across ALL active consonants.
    ...
    // **************************************************************
    // In the approach towards a consonantal target, the consonant
    // FULLY controls all VT parameters downstream from the place of
    // articulation. For example, the /sh/ gesture not only needs to
    // control the tongue tip/blade, but also the lip shape.
    // A consonant with a further anterior place of
    // articulation overwrites the targets of more posterior consonants.
    // In the return phase from a consonantal constriction,
    // the return gesture only controls the time constants.
    // Consonantal gestures set the time constant of ALL vocal tract
    // parameters, where the most anterior consonant has priority again.
    // **************************************************************

    // Start with the most posterior TONGUE BODY consonants.
    int type = 2;
    if (consonantExists[type])
    {
      targetValue[VocalTract::TCX] = consonantParam[type][VocalTract::TCX];
      targetValue[VocalTract::TCY] = consonantParam[type][VocalTract::TCY];
      targetValue[VocalTract::TBX] = consonantParam[type][VocalTract::TBX];
      targetValue[VocalTract::TBY] = consonantParam[type][VocalTract::TBY];
      targetValue[VocalTract::TTX] = consonantParam[type][VocalTract::TTX];
      targetValue[VocalTract::TTY] = consonantParam[type][VocalTract::TTY];
      targetValue[VocalTract::JX] = consonantParam[type][VocalTract::JX];
      targetValue[VocalTract::JA] = consonantParam[type][VocalTract::JA];
      targetValue[VocalTract::LP] = consonantParam[type][VocalTract::LP];
      targetValue[VocalTract::LD] = consonantParam[type][VocalTract::LD];
      targetValue[VocalTract::TS2] = consonantParam[type][VocalTract::TS2];
      targetValue[VocalTract::TS3] = consonantParam[type][VocalTract::TS3];
    }
    if ((consonantExists[type]) || (consonantName[type] == "RET"))
    {
      double tau = consonantTau_s[type];
      for (m = 0; m < VocalTract::NUM_PARAMS; m++)
      {
        targetTau[m] = tau;
      }
    ...
    // Then process a potential TONGUE TIP consonant.
    type = 1;
    if (consonantExists[type])
    {
      targetValue[VocalTract::TBX] = consonantParam[type][VocalTract::TBX];
    ...
    // Finally process the most anterior LABIAL consonants.
    type = 0;
    if (consonantExists[type])
    {
      targetValue[VocalTract::JX] = consonantParam[type][VocalTract::JX];
    ...
    // **************************************************************
    // Overwrite the velum targets by the velum gestures, if they
    // approach a non-neutral target or are in the return phase.
    // **************************************************************
    ...
    if (slice[i][VELIC_GESTURE].neutral == false)
    {
      ...
      if (slice[i][VELIC_GESTURE].sVal == "RET")
      {
        // In the return phase, only control the time constant.
        targetTau[VocalTract::VO] = slice[i][VELIC_GESTURE].tau_s;
      }

    // **************************************************************
    // Add the targets to the different target sequences.
    // **************************************************************
  ...
  for (i=0; i < numGestures; i++)
  {
    ...
    else
    {
      for (k=FIRST_SHAPE_PARAM; k < numGlottisParams; k++)
      {
        target.duration = gesture->duration_s;
        target.tau_s = gesture->tau_s;
        target.value = glottis->controlParams[k].neutral;
        target.slope = 0.0;
...
void GesturalScore::calcParamCurve(vector<Target> &paramTargets, vector<double> &paramCurve)
{
  ...
  for (i = 0; i <= finalPointIndex; i++)
  {
    ...
    while ((t_s > targetPos_s + target->duration) && (targetIndex < numTargets - 1))
    {
      ...
      // It's important to consider the slope for the first derivative f1 (=df/dt)!
      ...
      // ************************************************************
      // If a gesture with a big tau follows a gesture with a small 
      // tau, it may happen that the curve shape oscillates strongly
      // and overshoots the old or new targets, especially when the 
      // new gesture takes over at a point with a steep gradient.
      // The trick here is to reduce the derivatives by the ratio of
      // the old and new time constants!
      // ************************************************************

      if (target->tau_s > paramTargets[targetIndex - 1].tau_s)
      {
        // The ratio1 is always < 1.
        ratio1 = paramTargets[targetIndex - 1].tau_s / target->tau_s;
        ratio2 = ratio1 * ratio1;
        ratio3 = ratio2 * ratio1;
        ratio4 = ratio3 * ratio1;

        f1 *= ratio1;   // Reduce the gradient at the target boundary.
        f2 *= ratio2;   // Reduce the curvature at the target boundary.
        f3 *= ratio3;   // Reduce the higher order derivatives.
        f4 *= ratio4;
      }
...
// ****************************************************************************
// Calculate the time needed for the 5th-order dynamical system with the given
// time constant tau to reach the given normalized value (0 ... 1) in response 
// to a positive unit-step at t = 0.
// This function essentially approximates the inverse of the step response
// function.
// ****************************************************************************

double GesturalScore::getTimeToStepResponseValue_s(double normValue, double tau_s)
{
  const double EPSILON = 0.000000001;
  ...
  double c0 = -1.0;    // Must be the negative target value.
  double c1 = (-c0 * a);
  double c2 = (-c0 * a2 - c1 * a * 2) / 2;
  double c3 = (-c0 * a3 - c1 * a2 * 3 - c2 * a * 6) / 6;
  double c4 = (-c0 * a4 - c1 * a3 * 4 - c2 * a2 * 12 - c3 * a * 24) / 24;

  int i;
  double t, t2, t3, t4;

  double x = 0.0;     // Curve value.
  bool ok = false;
  double t_s = 0.0;
  double timeStep_s = 0.05 * tau_s;
  int maxTimeSteps = (int)(12.0 * tau_s / timeStep_s);
  ...
  for (i = 0; i <= maxTimeSteps; i++)
  {
    ...
    x = exp(a * t) * ((c0)+(c1)*t + (c2)*t2 + (c3)*t3 + (c4)*t4) + 1.0;

    // Leave the loop when the value is exceeded.
    if (x >= normValue)
    {
      break;
    }
  ...
  double timeToValue_s = t_s;   // The return value.
  return timeToValue_s;
}
...
void GesturalScore::setLowerLimit(double& x, const double minValue)
{
  if (x < minValue)
  {
    x = minValue;
...
void GesturalScore::setUpperLimit(double& x, const double maxValue)
{
  if (x > maxValue)
  {
    x = maxValue;
...
void GesturalScore::setLimits(double& x, const double minValue, const double maxValue)
{
  if (x < minValue)
  {
    x = minValue;
