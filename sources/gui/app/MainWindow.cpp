...
#include "util/IconsXpm.h"
#include "util/SilentMessageBox.h"
#include "vtl/io/SoundLib.h"
#include "vtl/io/AudioFile.h"
#include "vtl/dsp/Dsp.h"
#include "vtl/core/TimeFunction.h"
#include "vtl/io/XmlNode.h"
#include "vtl/synthesis/Synthesizer.h"
#include "vtl/api/VocalTractLabApi.h"
...
static const int IDM_EXPORT_TDS_SPECTRUM = 1238;
static const int IDM_EXPORT_EMA_TRAJECTORIES = 1240;
static const int IDM_EXPORT_TRACT_GLOTTIS_PARAMS = 1241;
static const int IDM_EXPORT_VIDEO_FRAMES     = 1242;
static const int IDM_EXPORT_TRANSFER_FUNCTIONS_FROM_SCORE = 1243;
static const int IDM_EXPORT_TRACT_CONTOURS_FROM_SCORE = 1244;
...
static const int IDM_SEGMENT_SEQUENCE_FILE_TO_AUDIO = 1260;
static const int IDM_GESTURAL_SCORE_FILE_TO_AUDIO = 1261;
static const int IDM_TUBE_SEQUENCE_FILE_TO_AUDIO = 1262;
static const int IDM_GESTURAL_SCORE_TO_TUBE_SEQUENCE_FILE = 1264;
static const int IDM_SEG_TO_WAV_FILES = 1265;

static const int IDM_HERTZ_TO_SEMITONES = 1270;  
static const int IDM_EXPERIMENT1 = 1271;
static const int IDM_EXPERIMENT2 = 1272;
static const int IDM_TEST1 = 1273;
static const int IDM_TEST2 = 1274;
static const int IDM_TRX_TRY_ESTIMATION = 1275;
...
BEGIN_EVENT_TABLE(MainWindow, wxFrame)
  ...
  EVT_MENU(IDM_EXPORT_TDS_SPECTRUM, MainWindow::OnExportTdsSpectrum)
  EVT_MENU(IDM_EXPORT_EMA_TRAJECTORIES, MainWindow::OnExportEmaTrajectories)
  EVT_MENU(IDM_EXPORT_TRACT_GLOTTIS_PARAMS, MainWindow::OnExportTractGlottisParams)
  ...
  EVT_MENU(IDM_EXPORT_TRACT_CONTOURS_FROM_SCORE, MainWindow::OnExportTractContoursFromScore)
  ...
  EVT_MENU(IDM_SEGMENT_SEQUENCE_FILE_TO_AUDIO, MainWindow::OnSegmentSequenceFileToAudio)
  ...
  EVT_MENU(IDM_SEG_TO_WAV_FILES, MainWindow::OnSegToWavFiles)

  EVT_MENU(IDM_HERTZ_TO_SEMITONES, MainWindow::OnHertzToSemitones)

  EVT_MENU(wxID_ABOUT, MainWindow::OnAbout)
  EVT_MENU(IDM_EXPERIMENT1, MainWindow::OnExperiment1)
  EVT_MENU(IDM_EXPERIMENT2, MainWindow::OnExperiment2)
  EVT_MENU(IDM_TEST1, MainWindow::OnTest1)
  EVT_MENU(IDM_TEST2, MainWindow::OnTest2)
  EVT_MENU(IDM_TRX_TRY_ESTIMATION, MainWindow::OnTrxTryEstimation)
...
MainWindow::MainWindow()
{
  ...
  initSound(AUDIO_SAMPLING_RATE_HZ);
  printf("done.\n");
...
void MainWindow::initWidgets()
{
  ...
  menu->Append(IDM_EXPORT_TDS_SPECTRUM, "Time-domain sim. (TDS) spectrum");
  menu->AppendSeparator();
  menu->Append(IDM_EXPORT_EMA_TRAJECTORIES, "EMA trajectories from ges. score (200 Hz)");
  menu->Append(IDM_EXPORT_TRACT_GLOTTIS_PARAMS, "Tract+glottis params from ges. score (250 Hz)");
  menu->Append(IDM_EXPORT_VIDEO_FRAMES, "Vocal tract video frames from ges. score (30 Hz)");
  menu->Append(IDM_EXPORT_TRANSFER_FUNCTIONS_FROM_SCORE, "Transfer functions from ges. score (1000 Hz)");
  menu->Append(IDM_EXPORT_TRACT_CONTOURS_FROM_SCORE, "Vocal tract contours from ges. score (100 Hz)");
  ...
  menu->Append(IDM_SEGMENT_SEQUENCE_FILE_TO_AUDIO, "Segment sequence file to audio");
  menu->Append(IDM_GESTURAL_SCORE_FILE_TO_AUDIO, "Gestural score file to audio");
  ...
  menu->Append(IDM_SEG_TO_WAV_FILES, "SEG to WAV (+BMP) files");
  ...
  menu->AppendSeparator();
  menu->Append(IDM_EXPERIMENT1, "Experiment 1")->Enable(false);
  menu->Append(IDM_EXPERIMENT2, "Experiment 2")->Enable(false);
  menu->Append(IDM_TEST1, "Test 1");
  menu->Append(IDM_TEST2, "Test 2")->Enable(false);
  menu->Append(IDM_TRX_TRY_ESTIMATION, "Data for TRX-TRY estimation")->Enable(false);
...
bool MainWindow::saveWaveformAsTxtFile(const wxString &fileName, Signal16 *signal, int pos, int length)
{
  ...
  {
    index = i*length/N;
    time = (double)index/(double)AUDIO_SAMPLING_RATE_HZ;
...
{
  double F0 = (double)AUDIO_SAMPLING_RATE_HZ / (double)spectrum->N;
  ...
  double freqSpacing_Hz = (double)AUDIO_SAMPLING_RATE_HZ / (double)spectrum->N;
...
void MainWindow::OnLoadWavEgg(wxCommandEvent &event)
{
  ...
  if (name.empty() == false)
  {
    ...
    if (audioFile.getSampleRate() != AUDIO_SAMPLING_RATE_HZ)
    {
      wxMessageBox(wxString::Format("Error: The audio file must have the sampling rate %d Hz.", (int)AUDIO_SAMPLING_RATE_HZ));
      return;
    }
    ...
    if (answer != wxCANCEL)
    {
      ...
      {
        data->track[Data::MAIN_TRACK]->setValue(targetPos + i, audioFile.samples[0][i] * 32767.0);
        data->track[Data::EGG_TRACK]->setValue(targetPos + i, audioFile.samples[1][i] * 32767.0);
      }
      updateWidgets();
    }
    ...
    if (trackIndex != -1)
    {
      ...
      if (answer == wxYES)
      {
        ...
        if (answer == wxNO)
        {
          ...
          {
            targetPos = (int)(data->gesturalScoreMark_s * AUDIO_SAMPLING_RATE_HZ);
          }
          else
          {
            targetPos = data->mark_pt;
      ...
      if (answer != wxCANCEL)
      {
        ...
        for (int i = 0; i < numSamples; i++)
        {
          data->track[trackIndex]->setValue(targetPos + i, audioFile.samples[0][i] * 32767.0);
        }
        updateWidgets();
      }
...
void MainWindow::OnExportTdsSpectrum(wxCommandEvent& event)
{
  wxFileName fileName(data->spectrumFileName);

  wxString name = wxFileSelector("Save TDS spectrum", fileName.GetPath(),
    fileName.GetFullName(), ".txt", "Text files (*.txt)|*.txt",
    wxFD_SAVE | wxFD_OVERWRITE_PROMPT, this);
  ...
  {
    data->spectrumFileName = name;
    saveSpectrum(name, data->tdsSpectrum);
  }
}
...
void MainWindow::OnExportEmaTrajectories(wxCommandEvent &event)
{
  ...
  if (name.empty() == false)
  {
    ...
    if (Synthesizer::gesturalScoreToEmaTrajectories(data->gesturalScore, name.ToStdString()) == false)
    {
      wxMessageBox("Saving the EMA trajectories failed.", "Error!");
    }
  }
}
...
void MainWindow::OnExportTractGlottisParams(wxCommandEvent& event)
{
  wxFileName fileName(data->tractGlottisParamsFileName);

  wxString name = wxFileSelector("Save tract and glottis parameter curves from gestural score", fileName.GetPath(),
    fileName.GetFullName(), ".txt", "Text files (*.txt)|*.txt",
    wxFD_SAVE | wxFD_OVERWRITE_PROMPT, this);

  if (name.empty() == false)
  {
    data->tractGlottisParamsFileName = name;

    if (data->exportTractGlottisParamCurves(name) == false)
    {
      wxMessageBox("Saving the tract and glottis parameter curves failed.", "Error!");
    }
  }
}
...
void MainWindow::OnExportTransferFunctionsFromScore(wxCommandEvent &event)
{
  ...
  if (name.empty() == false)
  {
    ...
    if (Synthesizer::gesturalScoreToTransferFunctions(data->gesturalScore, data->tlModel, name.ToStdString()) == false)
    {
      wxMessageBox("Saving the transfer functions failed.", "Error!");
    }
  }
}
...
void MainWindow::OnExportTractContoursFromScore(wxCommandEvent& event)
{
  wxDirDialog dialog(this, "Select a folder for the tract contour files");
  dialog.SetPath(data->tractContoursFolder);
  if (dialog.ShowModal() == wxID_OK)
  {
    data->tractContoursFolder = dialog.GetPath();
    string folderName = dialog.GetPath().ToStdString();
    if (Synthesizer::gesturalScoreToTractContourFiles(data->gesturalScore, folderName) == false)
    {
      wxMessageBox("Saving the vocal tract contour files failed.", "Error!");
    }
  }
}
...
void MainWindow::OnSegmentSequenceFileToAudio(wxCommandEvent& event)
{
  wxFileName fileName(data->segmentSequenceFileName);
  ...
  // Create a temporary segment sequence for the synthesis.

  SegmentSequence* ss = new SegmentSequence();

  if (ss->readFromFile(name.ToStdString()) == false)
  {
    wxMessageBox("Loading the segment sequence failed!", "Error!");
    delete ss;
    return;
  ...
  wxBusyInfo wait("Please wait...");
  gs->createFromSegmentSequence(ss);
  ...
  // Delete the temporary gestural score and segment sequence.
  delete gs;
  delete ss;
}
...
void MainWindow::OnTubeSequenceFileToAudio(wxCommandEvent &event)
{
  ...
  wxString name = wxFileSelector("Load tube sequence file", fileName.GetPath(),
    fileName.GetFullName(), ".tube", "Tube sequence files (*.tube)|*.tube",
    wxFD_OPEN | wxFD_FILE_MUST_EXIST, this);
...

  {
    ...
    wxBusyInfo wait(wxString("Please wait..."));
    ok = Synthesizer::synthesizeTubeSequence(name.ToStdString(), data->tdsModel, audio);
  }
...
// ****************************************************************************
// Generate WAV files (and potentially the vocal tract video frames) for all
// SEG files in a chosen folder.
// ****************************************************************************

void MainWindow::OnSegToWavFiles(wxCommandEvent& event)
{
  // Let the user choose the folder with the SEG files.
  wxDirDialog dialog(this, "Please select the folder with the SEG files", "", wxDD_DEFAULT_STYLE | wxDD_DIR_MUST_EXIST);
  if (dialog.ShowModal() != wxID_OK)
  {
    return;
  }

  wxString folderName = dialog.GetPath();
  wxChar pathSeparator = wxFileName::GetPathSeparator();
  if (folderName.EndsWith(&pathSeparator) == false)
  {
    folderName += pathSeparator;
  }

  // Find all SEG files in the folder.
  
  wxArrayString allSegFiles;
  wxDir::GetAllFiles(folderName, &allSegFiles, "*.seg", wxDIR_FILES);

  if (allSegFiles.IsEmpty())
  {
    wxMessageBox("There are no SEG files in the folder.", "Nothing to do...");
    return;
  }

  bool exportVideoFrames = false;

  if (wxMessageBox("Would you like to export the video frames during the synthesis?",
    "Export video frames?", wxYES_NO) == wxYES)
  {
    exportVideoFrames = true;
  }

  // ****************************************************************
  // Synthesize all SEG files in a loop.
  // ****************************************************************
  ...
  for (i = 0; i < allSegFiles.GetCount(); i++)
  {
    wxString segFileName = allSegFiles[i];
    wxString baseFileName = wxFileName(segFileName).GetName();   // Name without path and extension.
    wxString wavFileName = folderName + baseFileName + "-synth.wav";

//    wxPrintf("%s\n", baseFileName);

    // Load the segment sequence file.

    if (data->segmentSequence->readFromFile(segFileName.ToStdString()) == false)
    {
      wxPrintf(segFileName);
      printf("Error in MainWindow::OnSegToWavFiles(): Segment sequence file could not be loaded.\n");
      return;
    }

    // Generate, normalize and save the audio.

    wxPrintf("Generating audio file %s...\n", wavFileName);
    data->gesturalScore->createFromSegmentSequence(data->segmentSequence);
    
    // Extend the gestural score by a 400 ms pause (pressure gesture
    // with 0 Pa), which is nicer if you want to concatenate videos.
    ...
    data->gesturalScore->gestures[GesturalScore::PRESSURE_GESTURE].appendGesture(g);
    
    ok = data->synthesizer->synthesizeGesturalScore(
      data->gesturalScore, data->tdsModel, audio, "", true);

    data->normalizeAudioAmplitude(audio);
    if (data->saveWavFile(wavFileName, audio) == false)
    {
      printf("Error in MainWindow::OnSegToWavFiles(): WAV file could not be saved.\n");
      return;
    }

    // If selected, generate the video frames for this utterance.

    if (exportVideoFrames)
    {
      wxString imageFolder = folderName + baseFileName + "-images//";

      if (wxDirExists(imageFolder) == false)
      {
        // Try to create the folder.
        if (!wxMkdir(imageFolder)) //, 0777))
        {
          wxPrintf("Image folder %s could not be created.\n", imageFolder);
        }
      }
      else
      {
        // Try to clear the content of the existing folder.
        wxDir dir(imageFolder);
        if (dir.IsOpened())
        {
          wxPrintf("Deleting old files in the image folder %s.\n", imageFolder);
          wxString filename;
          bool doContinue = dir.GetFirst(&filename);
          while (doContinue)
          {
            wxFileName fullPath(imageFolder, filename);
            if (fullPath.FileExists())
            {
              if (!wxRemoveFile(fullPath.GetFullPath()))
              {
                wxPrintf("File in image folder could not be removed.\n");
              }
            }
            doContinue = dir.GetNext(&filename);
          }
        }
      }

      // Do the actual exporting of the image files.
      if (wxDirExists(imageFolder))
      {
        data->exportVocalTractVideoFrames(imageFolder);
      }
    }
...
void MainWindow::OnAbout(wxCommandEvent &event)
{
  ...
  wxMessageDialog dialog(this, 
    "This is VocalTractLab 2.4, developed by Peter Birkholz.\n\n" + st);
  dialog.ShowModal();
}
...
void MainWindow::OnExperiment1(wxCommandEvent& event)
{
  wxMessageBox("This experiment is currently not implemented!", "Info");
}
...
void MainWindow::OnExperiment2(wxCommandEvent& event)
{
  if (wxMessageBox("This experiment generates stimuli, which takes about 1.5 h. Continue?",
    "Continue", wxYES_NO) == wxYES)
  {
    // Gernerate stimuli for the experiment 2 (comparison of old and new glottis models).
    // Change the page to "Gestural score".
    notebook->SetSelection(3);
    wxYield();
    data->experiment2NewGlottisModel(this->gesturalScorePage);
  }
}
...
void MainWindow::OnTest1(wxCommandEvent& event)
{
/*
  if (wxMessageBox("This changes the glottis settings. Continue?",
...
/*
  ...
  vtlSegmentSequenceToGesturalScore("c:\\temp\\test1.seg", "c:\\temp\\test1.ges");
  vtlGesScoreChangeF0Offset("c:\\temp\\test1.ges", "c:\\temp\\test2.ges", 12.0);
  vtlGesScoreChangeDuration("c:\\temp\\test2.ges", "c:\\temp\\test3.ges", 2.0);
  vtlGesScoreChangeTimeConstants("c:\\temp\\test3.ges", "c:\\temp\\test4.ges", 1.5);
  */
...
// ****************************************************************************
// Synthesize a sequence of WAV files based on SEG files in a selected folder.
// ****************************************************************************

void MainWindow::OnTest2(wxCommandEvent& event)
{
  // ...
}
...
// ****************************************************************************
// Export the distance from the hyoid point to the tongue center along with 
// TRX and TRY for all raw vocal tract shapes to a TXT file, so that the 
// regression equations can be calculated from these data.
// ****************************************************************************

void MainWindow::OnTrxTryEstimation(wxCommandEvent& event)
{
  wxString st = "This function exports the distances from the hyoid "
    "to the tongue center along with TRX and TRY for all raw "
    "vocal tract shapes to a TXT file, so that the regression equations "
    "can be calculated from these data. Do you want to continue?";

  if (wxMessageBox(st, "Continue?", wxYES_NO) != wxYES)
  {
    return;
  }

  // ****************************************************************
  // Select a file name for the data to save.
  // ****************************************************************

  wxString fileName = wxFileSelector("Save regression data", "",
  ...
  if (ok == false)
  {
    wxMessageBox("File could not be opened!", "Error");
    return;
  }

  // Write the header.
  file.AddLine("shape_name, distance, TRX, TRY");
  ...
  int i, k;
  int numShapes = (int)tract->shapes.size();
  VocalTract::Shape* s;
  wxString line;
  ...
  for (i = 0; i < numShapes; i++)
  {
    ...
    // The shape name must contain "raw".
    if (s->name.find("raw") != string::npos)
    {
      for (k = 0; k < VocalTract::NUM_PARAMS; k++)
      {
        tract->params[k].x = s->param[k];
      }
      tract->calculateAll();

      // The tongue center point.
      Point2D C(tract->params[VocalTract::TCX].limitedX, tract->params[VocalTract::TCY].limitedX);
      
      // The position of the hyoid.
      Point2D H = tract->surfaces[VocalTract::LOWER_COVER].getVertex(VocalTract::NUM_LARYNX_RIBS - 1, VocalTract::NUM_LOWER_COVER_POINTS - 1).toPoint2D();
      
      // Find the hyoid - tongue center distance.
      double distance = (C - H).magnitude();
      
      // The TRX-TRY point.
      Point2D R(tract->params[VocalTract::TRX].x, tract->params[VocalTract::TRY].x);

      line = wxString::Format("%s, %f, %f, %f", s->name.c_str(), distance, R.x, R.y);
      file.AddLine(line);
    }
  }
