...
#include "dialogs/GlottisPanel.h"
...
void GlottisPanel::updateWidgets()
{
  ...
  for (i=0; i < (int)glottis->derivedParams.size(); i++)
  {
    st = wxString::Format("%2.2f %s", 
      glottis->derivedParams[i].x*glottis->derivedParams[i].factor,
      glottis->derivedParams[i].userUnit.c_str());
  ...
  for (i=0; i < (int)glottis->staticParams.size(); i++)
  {
    st = wxString::Format("%2.3f", glottis->staticParams[i].x*glottis->staticParams[i].factor);
...
void GlottisPanel::updateShapeList()
{
  ...
  int i;
  for (i=0; i < (int)glottis->shapes.size(); i++)
  {
    lstShape->Append(glottis->shapes[i].name);    
  }
}
...
void GlottisPanel::initWidgets()
{
  ...
  for (i=0; i < (int)glottis->staticParams.size(); i++)
  {
    st = wxString::Format("%s (%2.1f ... %2.1f %s)", 
      glottis->staticParams[i].name.c_str(),
      glottis->staticParams[i].min*glottis->staticParams[i].factor,
...
int GlottisPanel::getSelectedShapeIndex()
{
  ...
  for (i=0; i < (int)glottis->shapes.size(); i++)
  {
    if (wxString(glottis->shapes[i].name) == value)
    {
      k = i;
    }
  }
...
void GlottisPanel::OnSaveAsSelected(wxCommandEvent &event)
{
  ...
  if (k != -1)
  {
    wxString name = wxString( glottis->shapes[k].name );
    if (wxMessageBox("Do you really want to save the parameters as the shape <"+name+"> ?",
      "Save shape", wxYES_NO) == wxYES)
    {
      int numParams = (int)glottis->controlParams.size();
      glottis->shapes[k].controlParam.resize(numParams);
      for (i=0; i < numParams; i++)
      {
        glottis->shapes[k].controlParam[i] = glottis->controlParams[i].x;
      }
    }
  }
  else
...
void GlottisPanel::OnSaveAsExisting(wxCommandEvent &event)
{
  ...
  int numChoices = (int)glottis->shapes.size();
  if (numChoices < 1)
  {
  ...
  {
    choiceString[i] = wxString( glottis->shapes[i].name );
  }
  ...
  if (k != -1)
  {
    ...
    lstShape->SetValue( wxString(glottis->shapes[k].name) );
  }
}
...
void GlottisPanel::OnSaveAsNew(wxCommandEvent &event)
{
  ...
  for (i=0; i < (int)glottis->shapes.size(); i++)
  {
    if (wxString(glottis->shapes[i].name) == input)
    {
      k = i;
    }
  }
  ...
  if (k == -1)
  {
    ...
    for (i=0; i < numParams; i++)
    {
      s.controlParam[i] = glottis->controlParams[i].x;
    }

    glottis->shapes.push_back(s);
...
void GlottisPanel::OnRemove(wxCommandEvent &event)
{
  ...
  if (k != -1)
  {
    wxString value = wxString( glottis->shapes[k].name );
    if (wxMessageBox("Do you really want to remove the shape <"+value+"> ?",
      "Are you sure?", wxYES_NO) == wxYES)
    {
      glottis->shapes.erase( glottis->shapes.begin() + k );
      updateShapeList();
    }
  }
  else
...
void GlottisPanel::OnScrollEvent(wxScrollEvent &event)
{
  ...
  if ((index < 0) || (index >= (int)glottis->controlParams.size()))
  {
    return;
  }

  int pos = scrControlParam[index]->GetThumbPosition();
  glottis->controlParams[index].x = glottis->controlParams[index].min + 
    (pos/(double)NUM_SCROLL_STEPS)*(glottis->controlParams[index].max - glottis->controlParams[index].min);
...
void GlottisPanel::OnStaticParamChanged(wxFocusEvent &event)
{
  ...
  if ((i < 0) || (i > (int)glottis->staticParams.size()))
  {
    return;
  }

  Glottis::Parameter *p = &glottis->staticParams[i];
