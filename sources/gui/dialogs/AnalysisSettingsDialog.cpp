...
#include "dialogs/AnalysisSettingsDialog.h"
...
void AnalysisSettingsDialog::updateSpectrumPage()
{
  ...
  st = wxString::Format("%2.1f ms (%d pt)",
    1000.0*windowLength_pt / AUDIO_SAMPLING_RATE_HZ,
    windowLength_pt);
...
void AnalysisSettingsDialog::updateSpectrogramPage()
{
  ...
  st = wxString::Format("%2.1f ms (%d pt)",
    1000.0*((int)1 << frameLengthExponent) / AUDIO_SAMPLING_RATE_HZ,
...
{
  spectrogramPlot->windowLength_pt = (int)(0.006*AUDIO_SAMPLING_RATE_HZ);
