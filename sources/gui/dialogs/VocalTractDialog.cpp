...
void VocalTractDialog::updateWidgets()
{
  ...
  if (controlPanel->IsShown())
  {
    ...
    {
      x = tract->params[VocalTract::TS1+i].x;
      min = tract->params[VocalTract::TS1+i].min;
      max = tract->params[VocalTract::TS1+i].max;
...
void VocalTractDialog::OnTongueSideElevationChanged(wxScrollEvent &event)
{
  ...
  double min = tract->params[VocalTract::TS1+index].min;
  double max = tract->params[VocalTract::TS1+index].max;
  tract->params[VocalTract::TS1+index].x = min + (max-min)*(double)pos / (double)NUM_SCROLLBAR_STEPS;
