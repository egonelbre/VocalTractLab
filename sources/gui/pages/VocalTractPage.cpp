...
#include "pages/VocalTractPage.h"
...
#include "dialogs/TransitionDialog.h"
...
void VocalTractPage::updateWidgets()
{
  ...
  wxString st = wxString::Format("f0 = %2.0f Hz (speaker's default f0):", tract->anatomy.defaultF0_Hz);
  labDefaultF0->SetLabel(st);
...
void VocalTractPage::initWidgets(VocalTractPicture *picVocalTract)
{
  ...
  labDefaultF0 = new wxStaticText(this, wxID_ANY, "XXX");
  leftSizer->Add(labDefaultF0, 0, wxALL, 5);
  ...
  label = new wxStaticText(this, wxID_ANY, "f0 is controlled by LF model:");
  ...
  label = new wxStaticText(bottomPanel, wxID_ANY, "Noise filter cutoff freq.");
...
void VocalTractPage::createSpecialVocalTractShapes()
{
  ...
  double lpRange = vocalTract->params[VocalTract::LP].max - vocalTract->params[VocalTract::LP].min;
  ...
  for (i = 0; i < N; i++)
  {
      ...
      (name.find("-shorter") == string::npos))
    {
      ...
      if (newShape.param[index] < vocalTract->params[index].min)
      {
        newShape.param[index] = vocalTract->params[index].min;
      }

      index = VocalTract::LP;
      newShape.param[index] += lpDelta;
      if (newShape.param[index] > vocalTract->params[index].max)
      {
        newShape.param[index] = vocalTract->params[index].max;
      }
...
void VocalTractPage::OnMapShape(wxCommandEvent &event)
{
  ...
  for (i=0; i < VocalTract::NUM_PARAMS; i++)
  {
    vocalTractParams[i] = data->vocalTract->params[i].x;
  }
  ...
  for (i=0; i < VocalTract::NUM_PARAMS; i++)
  {
    vt->params[i].x = vt->shapes[shapeIndexA].param[i] + 
...
void VocalTractPage::OnPlayShortVowelTds(wxCommandEvent& event)
{
  ...
  Glottis* g = data->glottis[Data::GEOMETRIC_GLOTTIS_2025];
  int numGlottisParams = (int)g->controlParams.size();
  vector<double> audio;
  int i;
  ...
  {
    storedGlottisParams[i] = g->controlParams[i].x;
  }
  ...
  for (i = 0; i < numGlottisParams; i++)
  {
    if (s != NULL)
    {
      g->controlParams[i].x = s->controlParam[i];
    }
    else
    {
      g->controlParams[i].x = g->controlParams[i].neutral;
    }
  }
  g->controlParams[Glottis::FREQUENCY].x = data->vocalTract->anatomy.defaultF0_Hz;
  ...
  int duration_ms = (double)audio.size() * 1000.0 / (double)AUDIO_SAMPLING_RATE_HZ;
  ...
  for (i = 0; i < numGlottisParams; i++)
  {
    g->controlParams[i].x = storedGlottisParams[i];
  }
}
...
void VocalTractPage::OnSynthesizeNoise(wxCommandEvent &event)
{
  ...
  const int INPUT_SIGNAL_LENGTH = AUDIO_SAMPLING_RATE_HZ;    // For 1 s of noise
