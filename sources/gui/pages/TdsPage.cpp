...
static const int IDB_CALC_SUBGLOTTAL_IMPEDANCE  = 5006;
static const int IDB_CALC_SUPRAGLOTTAL_IMPEDANCE = 5007;
static const int IDB_CALC_TRANSFER_FUNCTION     = 5008;
static const int IDB_SET_AREA_FUNCTION          = 5009;
static const int IDB_SET_UNIFORM_TUBE           = 5010;
static const int IDB_GLOTTIS_DIALOG             = 5011;
static const int IDB_TDS_ACOUSTICS              = 5013;
static const int IDB_TRIANGULAR_MODEL_F0_PARAMS = 5014;
static const int IDC_SHOW_AREA_FUNCTION_ARTICULATORS = 5015;
...
BEGIN_EVENT_TABLE(TdsPage, wxPanel)
  ...
  EVT_BUTTON(IDB_CALC_SUBGLOTTAL_IMPEDANCE, TdsPage::OnCalcSubglottalImpedance)
  EVT_BUTTON(IDB_CALC_SUPRAGLOTTAL_IMPEDANCE, TdsPage::OnCalcSupraglottalImpedance)
  EVT_BUTTON(IDB_CALC_TRANSFER_FUNCTION, TdsPage::OnCalcTransferFunction)
...
void TdsPage::updateWidgets()
{
  ...
  wxString st = wxString::Format("%1.3f s", (double)data->nextTdsSampleIndex / (double)AUDIO_SAMPLING_RATE_HZ);
...
void TdsPage::initWidgets()
{
  ...
  button = new wxButton(this, IDB_CALC_SUBGLOTTAL_IMPEDANCE, "Calc. subglottal impedance");
  leftSizer->Add(button, 0, wxGROW | wxALL, 3);

  button = new wxButton(this, IDB_CALC_SUPRAGLOTTAL_IMPEDANCE, "Calc. supraglottal impedance");
  leftSizer->Add(button, 0, wxGROW | wxALL, 3);

  button = new wxButton(this, IDB_CALC_TRANSFER_FUNCTION, "Calc. transfer function");
  leftSizer->Add(button, 0, wxGROW | wxALL, 3);
...
void TdsPage::OnSynthesisThreadEvent(wxCommandEvent& event)
{
  ...
  if (n == -1)
  {
    ...
    if (data->nextTdsSampleIndex >= sequence->getDuration_pt())
    {
      synthesisComplete = true;
      data->nextTdsSampleIndex = 0;     // Reset
    }
    ...
    data->getSelectedGlottis()->controlParams[ Glottis::PRESSURE ].x = data->userPressure_dPa;
    data->getSelectedGlottis()->controlParams[ Glottis::FREQUENCY ].x = data->userF0_Hz;
  ...
  // The event asks us to update the progress dialog state
  {
    ...
    {
      TubeSequence *sequence = data->getSelectedTubeSequence();
      data->gesturalScoreMark_s = (double)data->nextTdsSampleIndex / (double)AUDIO_SAMPLING_RATE_HZ;
...
{
  data->nextTdsSampleIndex = 0;   // This means to restart the synthesis next time.
  data->synthesizer->reset();
...
{
  static TubeSequence* prevTubeSequence = nullptr;
  static Glottis* prevGlottis = nullptr;

  TubeSequence* tubeSequence = data->getSelectedTubeSequence();
  Glottis* glottis = data->getSelectedGlottis();

  // Reset the synthesis?
  if ((tubeSequence != prevTubeSequence) ||  (glottis != prevGlottis))
  {
    data->nextTdsSampleIndex = 0;
  }

  if (data->nextTdsSampleIndex == 0)
  {
    data->userPressure_dPa = glottis->controlParams[Glottis::PRESSURE].x;
    data->userF0_Hz = glottis->controlParams[Glottis::FREQUENCY].x;

    Tube tube;
    data->synthesizer->init(glottis, data->vocalTract, data->tdsModel);
    ...
    case Data::SYNTHESIS_PHONE:
      ...
      data->staticPhone->setup(tube, glottis, (int)(0.6 * AUDIO_SAMPLING_RATE_HZ));   // 0.6 s
      break;

    case Data::SYNTHESIS_GESMOD:
      // Set the currently selected glottis model.
      data->gesturalScore->glottis = glottis;
  ...
  // Keep in mind the type of synthesis and glottis.
  prevTubeSequence = tubeSequence;
  prevGlottis = glottis;
}
...
void TdsPage::OnCalcSubglottalImpedance(wxCommandEvent& event)
{
  vector<double> impulseResponse;

  Synthesizer::calcSubglottalImpedance(data->tdsModel, impulseResponse, data->tdsSpectrum);
  Synthesizer::normalizeAmplitude(impulseResponse);
  Synthesizer::copySignal(impulseResponse, *data->track[Data::MAIN_TRACK], 0);

  int duration_ms = (double)impulseResponse.size() * 1000.0 / (double)AUDIO_SAMPLING_RATE_HZ;
...
void TdsPage::OnCalcSupraglottalImpedance(wxCommandEvent& event)
{
  vector<double> impulseResponse;

  Synthesizer::calcSupraglottalImpedance(data->tdsModel, impulseResponse, data->tdsSpectrum);
  Synthesizer::normalizeAmplitude(impulseResponse);
  Synthesizer::copySignal(impulseResponse, *data->track[Data::MAIN_TRACK], 0);
...
void TdsPage::OnCalcTransferFunction(wxCommandEvent& event)
{
  vector<double> impulseResponse;

  Synthesizer::calcTransferFunction(data->tdsModel, impulseResponse, data->tdsSpectrum);
  Synthesizer::normalizeAmplitude(impulseResponse);
  Synthesizer::copySignal(impulseResponse, *data->track[Data::MAIN_TRACK], 0);
