...
#include "pages/GesturalScorePage.h"
...
void GesturalScorePage::fillGestureValueList(bool forced)
{
      ...
      ((data->selectedGestureType != prevSelectedGestureType) || (forced)))
  {
    ...
    if (data->selectedGestureType == GesturalScore::GLOTTAL_SHAPE_GESTURE)
    {
      ...
      {
        stringList.Add( wxString(glottis->shapes[i].name) );
      }
    }
...
void GesturalScorePage::OnSlopeChanged(wxScrollEvent &event)
{
  ...
  if (g != NULL)
  {
    ...
    {
      int pos = event.GetPosition();
      g->slope = sequence->minSlope + pos*(sequence->maxSlope - sequence->minSlope) / 1000.0;
...
void GesturalScorePage::OnSynthesize(wxCommandEvent &event)
{
  ...
  // Set the models for the gestural score and the synthesizer.
  ...
  synthesisThread = new SynthesisThread(this, data->gesturalScore);
...
void GesturalScorePage::OnSynthesisThreadEvent(wxCommandEvent& event)
{
  ...
  if (n == -1)
  {
    ...
    data->nextTdsSampleIndex = 0;     // Always reset
    ...
    if ((synthesisComplete) && (data->normalizeAmplitude))
    {
  ...
  // The event asks us to update the progress dialog state
  {
    ...
    if (data->showAnimation)
    {
      data->gesturalScoreMark_s = (double)data->tdsModel->getSampleIndex() / (double)AUDIO_SAMPLING_RATE_HZ;
