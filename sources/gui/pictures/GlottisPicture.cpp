...
{
  if (typeid(*glottis) == typeid(GeometricGlottis2025))
  {
    drawGeometricGlottis2025(dc);
  }
  else if (typeid(*glottis) == typeid(GeometricGlottis2019))
  {
    drawGeometricGlottis2019(dc);
  }
  else if (typeid(*glottis) == typeid(TriangularGlottis))
  {
...
void GlottisPicture::drawGeometricGlottis2025(wxDC &dc)
{
  // ****************************************************************
  // Fill the background.
  // ****************************************************************
  ...
  GeometricGlottis2025 *g = (GeometricGlottis2025*)glottis;
  ...
  double endX[2];
  double cordX[2];
  endX[0] = g->controlParams[GeometricGlottis2019::LOWER_END_X].x;
  endX[1] = g->controlParams[GeometricGlottis2019::UPPER_END_X].x;
  cordX[0] = g->derivedParams[GeometricGlottis2019::LOWER_CORD_X].x;
  cordX[1] = g->derivedParams[GeometricGlottis2019::UPPER_CORD_X].x;
  double length = g->derivedParams[GeometricGlottis2019::LENGTH].x;
  double thickness = g->derivedParams[GeometricGlottis2019::THICKNESS].x;
  double amplitude = g->derivedParams[GeometricGlottis2019::AMPLITUDE].x;
  ...
  double chinkLength = g->staticParams[GeometricGlottis2019::CHINK_LENGTH].x;
  double chinkWidth = g->derivedParams[GeometricGlottis2019::CHINK_WIDTH].x;
...
void GlottisPicture::drawGeometricGlottis2019(wxDC& dc)
{
  // ****************************************************************
  // Fill the background.
  // ****************************************************************
  ...
  GeometricGlottis2019* g = (GeometricGlottis2019*)glottis;
...
//  drawPolygon(dc, 4, p2);
  ...
  const double W = 0.3;
  double length = g->derivedParams[TriangularGlottis::CURRENT_LENGTH].x;
  
  double backX[2];
  backX[0] = g->controlParams[TriangularGlottis::REST_DISP_1].x + g->derivedParams[TriangularGlottis::RELATIVE_DISP_1].x;
  backX[1] = g->controlParams[TriangularGlottis::REST_DISP_2].x + g->derivedParams[TriangularGlottis::RELATIVE_DISP_2].x;
  ...
  if (g->controlParams[TriangularGlottis::REST_DISP_1].x < 0.0)
  {
    frontX[0] = backX[0];
  }
  else
  {
    frontX[0] = g->derivedParams[TriangularGlottis::RELATIVE_DISP_1].x;
  }
  
  if (g->controlParams[TriangularGlottis::REST_DISP_2].x < 0.0)
  {
    frontX[1] = backX[1];
  }
  else
  {
    frontX[1] = g->derivedParams[TriangularGlottis::RELATIVE_DISP_2].x;
  }


  double d[2];
  d[0] = g->derivedParams[TriangularGlottis::CURRENT_THICKNESS_1].x;
  d[1] = g->derivedParams[TriangularGlottis::CURRENT_THICKNESS_2].x;

  double chinkArea = g->controlParams[TriangularGlottis::ARY_AREA].x;
  if (chinkArea < 0.0)
  {
    chinkArea = 0.0;
  }
  ...
  double inletLength = g->staticParams[TriangularGlottis::INLET_LENGTH].x;
  double outletRadius = sqrt(2.5 / M_PI);
  double outletLength = g->staticParams[TriangularGlottis::OUTLET_LENGTH].x;
