...
void SpectrogramPicture::paintSpectrogram(wxDC &dc)
{
  ...
  double startTime_s  = (double)firstSample / (double)AUDIO_SAMPLING_RATE_HZ;
  double endTime_s    = (double)(firstSample + numSamples - 1) / (double)AUDIO_SAMPLING_RATE_HZ;
...
void SpectrogramPicture::OnMouseEvent(wxMouseEvent &event)
{
  ...
  if ((event.ButtonDown()) || (event.Dragging()))
  {
    ...
    // AND THIS PICTURE are refreshed!
