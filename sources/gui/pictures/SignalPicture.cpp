...
void SignalPicture::paintUpperOscillogram(wxDC &dc, int w, int h)
{
  ...
  if (data->showSpectrogramText)
  {
    ...
    st = wxString::Format(
      "View range: %d ms",
      (int)((double)data->oscillogramVisTimeRange_pt*1000.0 / AUDIO_SAMPLING_RATE_HZ));
    dc.DrawText(st, 0, 0);

    st = wxString::Format(
      "Mark pos.: %2.4f s",
      (double)data->mark_pt / (double)AUDIO_SAMPLING_RATE_HZ);
    ...
    if (data->isValidSelection())
    {
      ...
      double f = (double)AUDIO_SAMPLING_RATE_HZ / denominator;

      st = wxString::Format("Selection: %2.3f ... %2.3f s (%2.3f s = %2.1f Hz)",
        (double)left / (double)AUDIO_SAMPLING_RATE_HZ,
        (double)right / (double)AUDIO_SAMPLING_RATE_HZ,
        (double)(right-left) / (double)AUDIO_SAMPLING_RATE_HZ,
        f);
