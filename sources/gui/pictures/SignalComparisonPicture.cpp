...
static const int IDM_DELETE_SEGMENT_SEQUENCE = 9006;
...
BEGIN_EVENT_TABLE(SignalComparisonPicture, BasicPicture)
  ...
  EVT_MENU(IDM_DELETE_SEGMENT_SEQUENCE, SignalComparisonPicture::OnDeleteSegmentSequence)
  ...
  : BasicPicture(parent)
{
  ...
  contextMenu->Append(IDM_DELETE_SEGMENT_SEQUENCE, "Delete segment sequence");
...
void SignalComparisonPicture::paintSignals(wxDC &dc)
{
  ...
  int firstSample = startTime_s * (double)AUDIO_SAMPLING_RATE_HZ;
  int numSamples = duration_s * (double)AUDIO_SAMPLING_RATE_HZ;
  int index = 0;
  ...
  int x1 = data->gsTimeAxisGraph->getXPos(data->selectionMark_pt[0] / (double)AUDIO_SAMPLING_RATE_HZ);
  int x2 = data->gsTimeAxisGraph->getXPos(data->selectionMark_pt[1] / (double)AUDIO_SAMPLING_RATE_HZ);
  ...
  x1 = data->gsTimeAxisGraph->getXPos(data->selectionMark_pt[0] / (double)AUDIO_SAMPLING_RATE_HZ);
  x2 = data->gsTimeAxisGraph->getXPos(data->selectionMark_pt[1] / (double)AUDIO_SAMPLING_RATE_HZ);
...
void SignalComparisonPicture::OnPlaySegmentMainTrack(wxCommandEvent& event)
{
  ...
  if (index != -1)
  {
    int left = (int)(data->segmentSequence->getSegmentBegin_s(index) * AUDIO_SAMPLING_RATE_HZ);
    int right = (int)(data->segmentSequence->getSegmentEnd_s(index) * AUDIO_SAMPLING_RATE_HZ);

    waveStartPlaying(&data->track[Data::MAIN_TRACK]->x[left], right - left + 1, false);

    double duration_ms = 50 + (double)(right - left) * 1000.0 / AUDIO_SAMPLING_RATE_HZ;
...
// ****************************************************************************
/// Delete the whole segment sequence.
// ****************************************************************************

void SignalComparisonPicture::OnDeleteSegmentSequence(wxCommandEvent& event)
{
  SegmentSequence* sequence = data->segmentSequence;

  if (wxMessageBox("Do you want to delete the whole segment sequence?",
...
{
  data->selectionMark_pt[0] = (int)((double)AUDIO_SAMPLING_RATE_HZ * data->gsTimeAxisGraph->getAbsXValue(menuX));
  this->Refresh();
}
...
{
  data->selectionMark_pt[1] = (int)((double)AUDIO_SAMPLING_RATE_HZ * data->gsTimeAxisGraph->getAbsXValue(menuX));
  this->Refresh();
}
...
void SignalComparisonPicture::OnSelectAll(wxCommandEvent &event)
{
  ...
  data->selectionMark_pt[1] = Data::TRACK_DURATION_S*AUDIO_SAMPLING_RATE_HZ - 1;
  this->Refresh();
}
