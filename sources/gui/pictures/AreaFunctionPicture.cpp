...
void AreaFunctionPicture::draw(wxDC &dc)
{
  ...
  ts = model->tube.sections[Tube::LAST_MOUTH_SECTION];
...
//  printf("LAST_MOUTH_SECTION: x=%f  l=%f\n", ts->pos_cm, ts->length_cm);
  ...
  ts = model->tube.sections[Tube::LAST_NOSE_SECTION];
  ...
  if ((showAreas) && (showArticulators))    //(discreteAreaFunction) && 
  {
    ...
    {
      ts = &model->tube.pharynxMouthSections[i];
  ...
  if (showText)
  {
    ...
    if (showArticulators)
    {
      st = wxString::Format("Tongue tip sides: %2.2f", tract->params[VocalTract::TS3].x);
